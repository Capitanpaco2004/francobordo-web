...
/**
 * Lee xlsx Bluewave (header en fila 4, datos desde fila 5).
 * Devuelve mapa CODE -> ['cost', 'price'] (price ya redondeado al nickel).
 * Regla: precio = cost / PVP_DIVISOR.
 */
function loadBwPrices($path) {
    ...
    for ($r = 5; $r <= $hi; $r++) {
        ...
        if ($code === '') continue;
        // limpia "*" del code (variantes con bronce → mismo SKU base)
        $code = preg_replace('/[*\s]+$/', '', $code);
        $cost = bwParseNum($sheet->getCell('D' . $r)->getValue());
        if ($cost === null || $cost <= 0) continue;
        $cost = round($cost, 4);
        $price = roundToNickel($cost / PVP_DIVISOR);
...
<?php if ($isAction): ?>
    <h2>Actualizador precios Bluewave — <?php echo $dryRun ? 'PLAN (dry-run, sin cambios)' : 'EJECUCIÓN REAL'; ?></h2>
    <p><a href="<?php echo tep_href_link('Actualizador_precios_bluewave.php'); ?>" class="xbutton small hv9">← Volver</a></p>
...
$xlsx = findNewestXlsx(BW_DIR);
if (!$xlsx) { logMsg("ERROR: no hay xlsx en " . BW_DIR); goto end_action; }
logMsg("xlsx: " . basename($xlsx) . " (" . round(filesize($xlsx)/1024) . " KB, mtime " . date('Y-m-d H:i', filemtime($xlsx)) . ")");

logMsg("Cargando precios del xlsx (cost / " . PVP_DIVISOR . " → PVP neto)…");
$xlsxPrices = loadBwPrices($xlsx);
...
logMsg("Leyendo productos con products_import_origin LIKE 'bluewave%'…");
$prods = [];
$r = $mysqli->query("SELECT products_id, products_model, reference_prov, products_price, products_cost FROM products WHERE products_import_origin LIKE 'bluewave%'");
while ($row = $r->fetch_assoc()) $prods[(int) $row['products_id']] = $row;
logMsg("Productos Bluewave en BD: " . count($prods));
...
// ───────────────────── SPECIALS A BORRAR — cómputo (sólo si NO se aplican extremos) ─────────────────────
$badSpecials = [];
if (!$applyExtremes) {
    $effPrice = [];
...
try {
    ...
    // ───────────────────── SPECIALS A BORRAR — backup + DELETE + bump products_last_modified ─────────────────────
    if (!empty($badSpecials)) {
        $bakDir = '/home/francobordo/backups';
        @mkdir($bakDir, 0755, true);
        $bakPath = $bakDir . '/bluewave_specials_purge_' . date('Ymd_His') . '.sql';
        ...
        $fh = @fopen($bakPath, 'w');
        if ($fh) {
            fwrite($fh, "-- Backup specials borrados por Actualizador_precios_bluewave.php " . date('Y-m-d H:i:s') . "\n");
...
?>
    ...
    <p style="margin-top:15px;"><a href="<?php echo tep_href_link('Actualizador_precios_bluewave.php'); ?>" class="xbutton small hv9">← Volver</a></p>
<?php else: ?>
    <h2>Actualizador de precios — Bluewave</h2>
    <?php
        $xlsx = findNewestXlsx(BW_DIR);
    ?>
    <p>Relee el xlsx de <code><?php echo BW_DIR; ?></code> y actualiza los precios/cost/G1 de productos con <code>products_import_origin = 'bluewave'</code>.</p>
    <p>Fórmula: <code>PVP_neto = cost / <?php echo PVP_DIVISOR; ?></code> (margen 40% sobre PVP), redondeado a múltiplo 0.05 IVA inc.</p>
    <p>xlsx detectado: <?php echo $xlsx ? '<code>' . htmlspecialchars(basename($xlsx)) . '</code> (' . round(filesize($xlsx)/1024) . ' KB, ' . date('Y-m-d H:i', filemtime($xlsx)) . ')' : '<span style="color:red">NO ENCONTRADO</span>'; ?></p>

    <form method="get" action="<?php echo tep_href_link('Actualizador_precios_bluewave.php'); ?>" style="margin:10px 0;">
    ...
    <form method="get" action="<?php echo tep_href_link('Actualizador_precios_bluewave.php'); ?>" style="margin:10px 0;" onsubmit="return confirm('Confirmar EJECUCIÓN REAL en BD?');">

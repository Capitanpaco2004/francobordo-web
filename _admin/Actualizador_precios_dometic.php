...
/* ──────────────────────────────────────────────────────────────────────────
 * Actualizador de precios Dometic (y marcas del grupo: NDS, Büttner, Green Power, Dyno)
 *
 * Fuente ÚNICA = xlsx /import/Dometic/Lista de precios.xlsx (igual que el importador):
 *   - A=SKU · C=PVP (IVA NO incl.) · E=PRECIO NETO (coste) · F=EAN. C/E/F son fórmulas
 *     XLOOKUP → se leen sus valores cacheados (getOldCalculatedValue). Reutiliza la cache
 *     parsed.json del importador si existe.
 *   - products_price = roundToNickel(col C) · products_cost = col E · G1 = tiers+piso.
 *
 * Recalcula products_price / products_cost / G1. **Maneja productos CON variantes**: muchos
 * Dometic legacy (mfg 109) tienen atributos cuyas reference/EAN son los SKU/EAN del xlsx →
 * la variante más barata define el precio del padre y el resto son deltas. Stock NO se toca.
 * Scope: manufacturer Dometic/NDS/Büttner/Green Power/Dyno o products_import_origin 'dometic%'.
 * ────────────────────────────────────────────────────────────────────────── */

const DOM_DIR          = '/home/francobordo/public_html/import/Dometic/';
const DOM_CACHE_DIR    = DOM_DIR . 'cache/';
const DOM_PARSE_CACHE  = DOM_CACHE_DIR . 'parsed.json';   // compartida con el importador
const G1_GROUP_ID      = 1;
...
const MFG_NAMES        = ['Dometic', 'NDS', 'Büttner', 'Green Power', 'Dyno'];
...
$reparse       = isset($_POST['reparse']) || isset($_GET['reparse']);
...
/** Parsea el xlsx Dometic → filas [sku, pvp(col C), cost(col E), ean(col F)].
 *  Reutiliza la cache parsed.json del importador (clave nombre:mtime:size) si está fresca. */
function domLoadRows($xlsx, $reparse, &$msg) {
    $msg = '';
    $sig = basename($xlsx) . ':' . filemtime($xlsx) . ':' . filesize($xlsx);
    if (!$reparse && file_exists(DOM_PARSE_CACHE)) {
        $c = json_decode((string) file_get_contents(DOM_PARSE_CACHE), true);
        if (is_array($c) && ($c['sig'] ?? '') === $sig && isset($c['rows']) && is_array($c['rows'])) { $msg = 'parsed.json (importador)'; return $c['rows']; }
    }
    try {
        $reader = IOFactory::createReaderForFile($xlsx);
        $reader->setReadDataOnly(false);
        $sh = $reader->load($xlsx)->getSheet(0);
    } catch (Throwable $e) { $msg = 'ERROR xlsx: ' . $e->getMessage(); return []; }
    $hr = $sh->getHighestRow(); $rows = [];
    for ($r = 5; $r <= $hr; $r++) {
        $a = trim((string) $sh->getCell("A$r")->getValue());
        if (!preg_match('/^\d{6,}$/', $a)) continue;
    ...
    $msg = 'xlsx (' . count($rows) . ' filas, sin cache)';
    return $rows;
}

/** rows → [bySku, byEan] con ['price'=>roundToNickel(C), 'cost'=>E]. */
function domBuildMaps(array $rows) {
    $bySku = []; $byEan = [];
    foreach ($rows as $row) {
        if ($row['pvp'] === null || $row['pvp'] <= 0) continue;
        $rec = ['price' => roundToNickel($row['pvp']), 'cost' => round((float) $row['cost'], 4)];
        $bySku[strtolower(trim((string) $row['sku']))] = $rec;
...
/** Match: price+cost por SKU (modelo/ref) o, si no, por EAN. Ambos vienen de la misma fila. */
function domMatch($bySku, $byEan, array $skuCands, array $eanCands) {
...
<?php if ($isAction): ?>
    <h2>Actualizador precios Dometic — <?php echo $dryRun ? 'PLAN (dry-run, sin cambios)' : 'EJECUCIÓN REAL'; ?></h2>
    <p><a href="<?php echo tep_href_link('Actualizador_precios_dometic.php'); ?>" class="xbutton small hv9">← Volver</a></p>
...
$xlsx = findNewestXlsx(DOM_DIR);
if (!$xlsx) { logMsg("ERROR: no hay xlsx en " . DOM_DIR); goto end_action; }
logMsg("xlsx: " . basename($xlsx) . " (" . round(filesize($xlsx)/1024) . " KB, mtime " . date('Y-m-d H:i', filemtime($xlsx)) . ")");

$srcMsg = '';
$rows = domLoadRows($xlsx, $reparse, $srcMsg);
if (empty($rows)) { logMsg("ERROR cargando xlsx: $srcMsg"); goto end_action; }
list($bySku, $byEan) = domBuildMaps($rows);
logMsg("Precios del xlsx [$srcMsg]: " . count($bySku) . " por SKU / " . count($byEan) . " por EAN (price=roundToNickel(C), cost=E)");

$mysqli = new mysqli(DB_SERVER, DB_SERVER_USERNAME, DB_SERVER_PASSWORD, DB_DATABASE);
if ($mysqli->connect_error) { logMsg("ERROR DB: " . $mysqli->connect_error); goto end_action; }
$mysqli->set_charset('utf8');

// scope: manufacturers Dometic/NDS/Büttner/Green Power/Dyno + origin dometic%
$mfgIds = [];
$inNames = implode(',', array_map(fn($n) => "'" . $mysqli->real_escape_string($n) . "'", MFG_NAMES));
$r = $mysqli->query("SELECT manufacturers_id, manufacturers_name FROM manufacturers WHERE manufacturers_name IN ($inNames)");
while ($r && $row = $r->fetch_assoc()) $mfgIds[] = (int) $row['manufacturers_id'];
$scopeSql = "p.products_import_origin LIKE '" . ORIGIN_FLAG . "%'";
if (!empty($mfgIds)) $scopeSql = "(" . $scopeSql . " OR p.manufacturers_id IN (" . implode(',', $mfgIds) . "))";
logMsg("Fabricantes en scope: " . (empty($mfgIds) ? "(ninguno aún)" : implode(',', $mfgIds)) . " + origin '" . ORIGIN_FLAG . "%'");
...
foreach ($prods as $pid => $p) {
    ...
    if (empty($variants)) {
        // ── Producto SUELTO ──
        $entry = domMatch($bySku, $byEan, [$p['products_model'], $p['reference_prov']], [$p['product_ean']]);
    ...
    // ── Producto CON VARIANTES ── (legacy Dometic con atributos)
    $variantPrices = []; $missing = [];
    foreach ($variants as $a) {
        $entry = domMatch($bySku, $byEan, [$a['reference'], $a['reference_prov']], [$a['products_attributes_ean']]);
...
// ───────────────────── Specials a borrar cuando NO se aplican extremos ─────────────────────
...
logMsg("Sin match en el xlsx : " . count($noMatch));
...
if (!$onlyExtremes && !empty($noMatch)) {
    logMsg("--- Sin match en el xlsx (TODOS: " . count($noMatch) . ", no se tocan) ---");
...
try {
    ...
    if (!empty($badSpecials)) {
        ...
        $bakPath = $bakDir . '/dometic_specials_purge_' . date('Ymd_His') . '.sql';
        ...
        $fh = @fopen($bakPath, 'w');
        if ($fh) {
            fwrite($fh, "-- Backup specials borrados por Actualizador_precios_dometic.php " . date('Y-m-d H:i:s') . "\n");
...
?>
    ...
    <p style="margin-top:15px;"><a href="<?php echo tep_href_link('Actualizador_precios_dometic.php'); ?>" class="xbutton small hv9">← Volver</a></p>
<?php else: ?>
    <h2>Actualizador de precios — Dometic (+ NDS / Büttner / Green Power / Dyno)</h2>
    <?php $xlsx = findNewestXlsx(DOM_DIR); if (!$xlsx) echo '<p style="color:red;">No hay xlsx en ' . DOM_DIR . '</p>'; else echo '<p style="color:#666;font-size:13px;">xlsx: <code>' . htmlspecialchars(basename($xlsx)) . '</code> (' . round(filesize($xlsx)/1024) . ' KB, mtime ' . date('Y-m-d H:i', filemtime($xlsx)) . ')</p>'; ?>
    <p>
        Recalcula precios de los productos Dometic y marcas del grupo (manufacturer Dometic/NDS/Büttner/Green&nbsp;Power/Dyno o <code>origin dometic%</code>):
        <strong>PVP</strong> ← <code>roundToNickel(col C)</code> del xlsx (col C es PVP sin IVA), <strong>coste</strong> ← col E (neto),
        enlazado por <strong>SKU</strong> (modelo/referencia ↔ col A; coste/precio también por EAN col F).
        <strong>Maneja productos legacy con variantes</strong> (la más barata define el precio del padre y el resto el delta).
    ...
    <form method="post" style="background:#f5f5f5;padding:15px;border-radius:5px;">
        ...
        <p><label><input type="checkbox" name="reparse" value="1"> Re-parsear el xlsx (ignora la cache parsed.json del importador)</label></p>
    ...
    <p style="margin-top:20px;color:#888;font-size:12px;">
        <strong>Reglas</strong> (idénticas al importador Dometic):<br>
        - <code>products_price</code> ← <code>roundToNickel(col C)</code> (col C = PVP sin IVA del xlsx).<br>
        - <code>products_cost</code> ← col E (precio neto).<br>
        - <strong>Grupo 1</strong>: tiers según margen (≥45% ×0.75, 40 ×0.80, 35 ×0.82, 30 ×0.85, &lt;30% ×0.90) + piso cost×<?php echo G1_FLOOR_FACTOR; ?>.<br>
        - Match por <strong>SKU</strong> (modelo/ref ↔ col A; variantes por su reference) y por EAN (col F). Ambos (price+cost) salen de la misma fila del xlsx.<br>
        - Variantes (productos Dometic legacy con atributos): padre = más barata, resto delta + delta G1. Sin match → se lista, no se toca. Stock NO se toca.
    </p>
<?php endif; ?>
</div></td></tr></table>

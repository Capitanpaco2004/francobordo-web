...
/* ──────────────────────────────────────────────────────────────────────────
 * Actualizador de precios KENT (Alliance Marine)
 *
 * Fuente ÚNICA = CSV del SFTP de Alliance Marine (el mismo que el importador):
 *   /import/feed/KENT/KENT_products_francobordo.csv — pipe |, UTF-8, con cabecera.
 ...
 * Scope: SOLO products_import_origin 'kent%' (multi-marca: las marcas de KENT se comparten
...
const KENT_SFTP_HOST    = 'stftpamgprd01.blob.core.windows.net';
...
function kentNormalizeEan($raw) { $e = preg_replace('/\D/', '', trim((string) $raw)); if (strlen($e) === 12) $e = '0' . $e; elseif (strlen($e) === 14 && $e[0] === '0') $e = substr($e, 1); return isValidEan13($e) ? $e : ''; }
function kentParseNum($v) { $v = trim((string) $v); if ($v === '') return null; if (strpos($v, ',') !== false && strpos($v, '.') === false) $v = str_replace(',', '.', $v); $v = str_replace(' ', '', $v); return is_numeric($v) ? (float) $v : null; }
 ...
 *  Incluye Active y End of life (mientras KENT publique precio, se actualiza). */
function kentBuildMaps($path, &$rowsTotal = 0, &$rowsPriced = 0) {
    $f = fopen($path, 'r');
    if (!$f) return [[], []];
    ...
    while (($r = fgetcsv($f, 0, '|', chr(34), '')) !== false) {
        ...
        if ($sku === '') continue;
        $net = kentParseNum($r[$idx['Net_price_ex_VAT']] ?? '');
        if ($net === null || $net <= 0) continue;
        $pub = kentParseNum($r[$idx['Public_price_ex_VAT']] ?? '');
        ...
        $ean = kentNormalizeEan($r[$idx['Barcode']] ?? '');
...
function kentMatch($bySku, $byEan, array $skuCands, array $eanCands) {
...
<?php if ($isAction): ?>
    <h2>Actualizador precios KENT — <?php echo $dryRun ? 'PLAN (dry-run, sin cambios)' : 'EJECUCIÓN REAL'; ?></h2>
    <p><a href="<?php echo tep_href_link('Actualizador_precios_kent.php'); ?>" class="xbutton small hv9">← Volver</a></p>
...
if (!$skipDownload) {
    ...
    logMsg("Descargando " . KENT_SFTP_REMOTE . " del SFTP…" . ($mtime ? " (local actual: " . date('Y-m-d H:i', $mtime) . ")" : ""));
    $dl = downloadFeedFromSftp(KENT_CSV);
    if ($dl['ok']) logMsg("  ✓ descargado: " . round($dl['size']/1048576, 1) . " MB");
    else { logMsg("  ✗ descarga fallida: " . $dl['err'] . " — uso copia local si la hay"); if (!file_exists(KENT_CSV)) { logMsg("ERROR: no hay copia local"); goto end_action; } }
}
if (!file_exists(KENT_CSV)) { logMsg("ERROR: CSV no encontrado: " . KENT_CSV); goto end_action; }
logMsg("CSV: " . basename(KENT_CSV) . " (" . round(filesize(KENT_CSV)/1048576, 1) . " MB, mtime " . date('Y-m-d H:i', filemtime(KENT_CSV)) . ")");

$rowsTotal = 0; $rowsPriced = 0;
list($bySku, $byEan) = kentBuildMaps(KENT_CSV, $rowsTotal, $rowsPriced);
...
foreach ($prods as $pid => $p) {
    ...
    if (empty($variants)) {
        // ── Producto SUELTO ──
        $entry = kentMatch($bySku, $byEan, [$p['reference_prov'], $p['products_model']], [$p['product_ean']]);
    ...
    foreach ($variants as $a) {
        $entry = kentMatch($bySku, $byEan, [$a['reference_prov'], $a['reference']], [$a['products_attributes_ean']]);
...
?>
    ...
    <p style="margin-top:15px;"><a href="<?php echo tep_href_link('Actualizador_precios_kent.php'); ?>" class="xbutton small hv9">← Volver</a></p>
<?php else: ?>
    <h2>Actualizador de precios — KENT (Alliance Marine)</h2>
    <?php if (file_exists(KENT_CSV)) echo '<p style="color:#666;font-size:13px;">CSV local: <code>' . htmlspecialchars(basename(KENT_CSV)) . '</code> (' . round(filesize(KENT_CSV)/1048576, 1) . ' MB, mtime ' . date('Y-m-d H:i', filemtime(KENT_CSV)) . ') — se re-descarga del SFTP en cada run salvo que marques lo contrario.</p>'; ?>
    <p>
        Recalcula precios de los productos con <code>origin kent%</code> desde el CSV de Alliance Marine
        (precios negociados, se regeneran cada medianoche): <strong>PVP</strong> ← <code>roundToNickel(Public_price_ex_VAT)</code>
        (fallback coste×2), <strong>coste</strong> ← <code>Net_price_ex_VAT</code>, enlazado por <strong>SKU KENT</strong>
    ...
    <p style="margin-top:20px;color:#888;font-size:12px;">
        <strong>Reglas</strong> (idénticas al importador KENT):<br>
        ...
        - Match por SKU KENT (reference_prov), ref. fabricante (products_model / reference) y EAN. Incluye filas Active y End of life del feed.<br>
        - Variantes: padre = más barata, resto delta + delta G1. Producto/variante sin match → se lista, no se toca.<br>
        - Scope SOLO <code>origin kent%</code> (las marcas de KENT se comparten con otros proveedores). Stock y specials NO se tocan.
    </p>
<?php endif; ?>
</div></td></tr></table>

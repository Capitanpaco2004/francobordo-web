...
/**
 * Lee el xlsx Lankhorst y devuelve mapa Code (col A) → ['cost', 'price'].
 * Reglas (idénticas al importador):
 *  - cost  = Wholesale_Export 2026 (col F, sin IVA)
 *  - price = RRP No VAT (col D, sin IVA); fallback cost × 2 si vacío o < cost
 *  - solo VP=STK (col C). Otras filas se ignoran.
 */
function loadLankPrices($path) {
	...
	for ($r = 2; $r <= $hi; $r++) {
		...
		if ($vp !== 'STK') continue;
		$rrp  = lankParseNum($sheet->getCell('D' . $r)->getValue());
		$whol = lankParseNum($sheet->getCell('F' . $r)->getValue());
		if ($whol === null || $whol <= 0) continue;
		$cost  = round($whol, 4);
		$price = ($rrp !== null && $rrp > $cost) ? $rrp : $cost * 2.0;
		$prices[$code] = ['cost' => $cost, 'price' => roundToNickel($price)];
	}
	return $prices;
}
...
<?php if ($isAction): ?>
	<h2>Actualizador precios Lankhorst — <?php echo $dryRun ? 'PLAN (dry-run, sin cambios)' : 'EJECUCIÓN REAL'; ?></h2>
	<p><a href="<?php echo tep_href_link('Actualizador_precios_lankhorst.php'); ?>" class="xbutton small hv9">← Volver</a></p>
...
logMsg("Cargando precios del xlsx (solo VP=STK)…");
$xlsxPrices = loadLankPrices($xlsx);
...
logMsg("Leyendo productos con products_import_origin LIKE 'lankhorst%'…");
$prods = [];
$r = $mysqli->query("SELECT products_id, products_model, reference_prov, products_price, products_cost FROM products WHERE products_import_origin LIKE 'lankhorst%'");
while ($row = $r->fetch_assoc()) $prods[(int) $row['products_id']] = $row;
logMsg("Productos Lankhorst en BD: " . count($prods));
...
// Variantes (products_attributes) — opcional, productos con familia
$attrsByProd = [];
...
foreach ($prods as $pid => $p) {
	...
	if (empty($variants)) {
		...
		$processed++;
		$newCost  = $entry['cost'];
		$newPrice = $entry['price']; // ya redondeado
...
// Política V3 (2026-07-17): solo ofertas de productos cuyo PVP cambia en ESTE run ($updPriceMain).
...
try {
	...
	if (!empty($badSpecials)) {
	    ...
	    // Bump products_last_modified de pids cuyos specials se borraron (cachés/feeds)
...
?>
	...
	<p style="margin-top:15px;"><a href="<?php echo tep_href_link('Actualizador_precios_lankhorst.php'); ?>" class="xbutton small hv9">← Volver</a></p>
<?php else: ?>
	<h2>Actualizador de precios — Lankhorst</h2>
	<?php
		$xlsx = findNewestXlsx(LANK_DIR);
		if (!$xlsx) {
	...
	?>
	<p>
		Compara <code>Wholesale_Export</code> y <code>RRP No VAT</code> del xlsx Lankhorst con
		<code>products.products_cost</code>, <code>products_price</code> y <code>products_groups</code> (Grupo 1)
		para los productos con <code>products_import_origin LIKE 'lankhorst%'</code>.
	...
	<p style="margin-top:20px;color:#888;font-size:12px;">
		<strong>Reglas aplicadas</strong> (idénticas al importador Lankhorst):<br>
		- <code>products_cost</code>  ← Wholesale_Export 2026 (col F, sin IVA).<br>
		- <code>products_price</code> ← <code>roundToNickel(RRP No VAT)</code> (col D); fallback <code>cost × 2</code> si vacío o &lt; cost.<br>
		...
		- Solo filas con <code>VP=STK</code> en el xlsx. Variantes: padre = variante más barata, resto con delta.<br>

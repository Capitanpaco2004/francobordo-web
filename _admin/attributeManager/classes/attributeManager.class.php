...
/*abstract*/ class attributeManager /*implements attributeManagerInterface*/ {
	...
	function addOptionValue($get) {
		...
		foreach($arrOptionValues as $optionValue) {
			...
			if ($newId == 0)
			{
				$aMax = amDB::query("select MAX( products_options_values_id ) AS products_options_values_id_max from " . TABLE_PRODUCTS_OPTIONS_VALUES );
				$aMax = amDB::fetchArray( $aMax );
				$newId = $aMax['products_options_values_id_max'] + 1;
			}
	...
	function getAllTemplatesAndAttributes() {
		if(0 === count($this->arrAllTemplatesAndAttributes)) {
			...
			foreach($ref as $res) {
				...
				$optionId = $res['options_id'] ?? '';
				$optionValueId = $res['option_values_id'] ?? '';
				if ($optionId === '' && $optionValueId === '') continue;

				$this->arrAllTemplatesAndAttributes[$templateID][$optionId][] = $optionValueId;

				if (AM_USE_SORT_ORDER) {
          			$this->arrAllTemplatesAndAttributes[$templateID]['sortOrder'][$optionValueId] = $res[AM_FIELD_OPTION_VALUE_SORT_ORDER];
  				}
  				// Added by Red Earth Design, Inc. to populate price and prefix
  				$this->arrAllTemplatesAndAttributes[$templateID]['price_prefix'][$optionValueId] = $res['price_prefix'];
  				$this->arrAllTemplatesAndAttributes[$templateID]['options_values_price'][$optionValueId] = $res['options_values_price'];
				$this->arrAllTemplatesAndAttributes[$templateID]['reference'][$optionValueId] = $res['reference'];
				$this->arrAllTemplatesAndAttributes[$templateID]['reference_prov'][$optionValueId] = $res['reference_prov'];
				$this->arrAllTemplatesAndAttributes[$templateID]['products_attributes_ean'][$optionValueId] = $res['products_attributes_ean'];
                if (AM_USE_MPW) {
                  $this->arrAllTemplatesAndAttributes[$templateID]['weight_prefix'][$optionValueId] = $res['price_weight'];
                  $this->arrAllTemplatesAndAttributes[$templateID]['options_values_weight'][$optionValueId] = $res['options_values_weight'];
                }
           }
		}
					...
					foreach ($values['values'] as $optionValuesId => $allOptionValues){
						if (!AM_USE_SORT_ORDER) {
  							$data = array(
  								...
  								'reference'=> $values['values'][$optionValuesId]['reference'],
								'reference_prov'=> $values['values'][$optionValuesId]['reference_prov'],
								'products_attributes_ean'=> $values['values'][$optionValuesId]['products_attributes_ean']
  							);
  						} else {
	  						$data = array(
  								'template_id' => $newTemplateId,
...
//								  echo 'TempID '.$templateId . ' OPTvalID ' . $optionValuesId . ' ';
									$this->addAttributeToProduct(
				                      ...
				                      'weight' => $allTemplatesAttributes[$templateId]['options_values_weight'][$optionValuesId],
                      'weight_prefix' => $allTemplatesAttributes[$templateId]['weight_prefix'][$optionValuesId],
						'reference' => $allTemplatesAttributes[$templateId]['reference'][$optionValuesId],
						'reference_prov' => $allTemplatesAttributes[$templateId]['reference_prov'][$optionValuesId],
						'products_attributes_ean' => $allTemplatesAttributes[$templateId]['products_attributes_ean'][$optionValuesId],
								  )
									);
								}
								...
								if (AM_USE_QT_PRO && AM_USE_SORT_ORDER) {
									$this->addAttributeToProduct(
									  array(
									    ...
									    'sortOrder' => $allTemplatesAttributes[$templateId]['sortOrder'][$optionValuesId],
									    'stockTracking' => '0',
								//		'reference' => $allTemplatesAttributes[$templateId]['reference'][$optionValuesId]  // JFJ - artikelnr per attribut
									                      'weight' => $allTemplatesAttributes[$templateId]['options_values_weight'][$optionValuesId],
                      'weight_prefix' => $allTemplatesAttributes[$templateId]['weight_prefix'][$optionValuesId]

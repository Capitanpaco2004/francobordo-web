...
// JFJ - artikelnr per attribut BOF
  if(isset($get['reference']) === true  ) {
    $this->getAndPrepare('reference', $get, $getArray['reference']);
  }

  if(isset($get['reference_prov']) === true  ) {
    $this->getAndPrepare('reference_prov', $get, $getArray['reference_prov']);
  }

  if(isset($get['products_attributes_ean']) === true  ) {
    $this->getAndPrepare('products_attributes_ean', $get, $getArray['products_attributes_ean']);
  }
...
// QT pro
	...
	function update($get) {
		...
		foreach($this->arrSessionVar as $id => $res) {
			if(($res['option_id'] == $getArray['option_id']) && ($res['option_value_id'] == $getArray['option_value_id'])) {
				...
				$this->arrSessionVar[$id]['reference'] = $getArray['reference'];
				$this->arrSessionVar[$id]['reference_prov'] = $getArray['reference_prov'];
				$this->arrSessionVar[$id]['products_attributes_ean'] = $getArray['products_attributes_ean'];
	...
	function getAllProductOptionsAndValues($reset = false) {
		...
		for($z1=0;$z1<count($this->arrSessionVar);$z1++){
			...
			foreach($this->arrSessionVar as $id => $res) {
                ...
                $this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['reference'] = $res['reference'];
				$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['reference_prov'] = $res['reference_prov'];
				$this->arrAllProductOptionsAndValues[$optionsId]['values'][$res['option_value_id']]['products_attributes_ean'] = $res['products_attributes_ean'];
                if (AM_USE_MPW) {
	...
	function sortProductAttributes(){
		...
		foreach( $result as $key => $currentOption ) {
			foreach( $currentOption['values'] as $currentOptionValuekey => $currentOptionValues ) {
					...
					$sortedArray[$sortorder] =  array( 'optionValue-id' => $currentOptionValuekey,
														...
														'reference' => $currentOptionValues['reference'],
														'reference_prov' => $currentOptionValues['reference_prov'],
														'products_attributes_ean' => $currentOptionValues['products_attributes_ean'],
		...
		foreach( $sortedArray as $key => $currentOption ) {
			...
			$optionValues[$currentOption['optionValue-id']] = array(
																	...
																	'reference' => $currentOption['optionValue-reference'],

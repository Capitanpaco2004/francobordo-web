                                        ...
                                        <?php
                                            ...
                                            else
                                            {
                                                if ($admin_cat_access == "ALL")
                                                    $categories_query = tep_db_query("select c.categories_id, COALESCE(NULLIF(cd.categories_name,''), NULLIF(cden.categories_name,''), CONCAT('[sin nombre #', c.categories_id, ']')) as categories_name, cad.categories_name as categories_name_amazon, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified, c.categories_status from " . TABLE_CATEGORIES . " c LEFT JOIN " . TABLE_CATEGORIES_DESCRIPTION . " cd on( c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' ) LEFT JOIN " . TABLE_CATEGORIES_DESCRIPTION . " cden on( c.categories_id = cden.categories_id and cden.language_id = 1 ) LEFT JOIN categories_amazon_description cad on (c.categories_amazon_id = cad.categories_id and cad.language_id = '" . (int)$languages_id . "') where c.parent_id = '" . (int)$current_category_id . "' order by c.sort_order, categories_name");
                                                //else if ($admin_cat_access == "")
                                                    //$categories_query = tep_db_query("");
                                                else
                                                    $categories_query = tep_db_query("select c.categories_id, COALESCE(NULLIF(cd.categories_name,''), NULLIF(cden.categories_name,''), CONCAT('[sin nombre #', c.categories_id, ']')) as categories_name, cad.categories_name as categories_name_amazon, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified, c.categories_status from " . TABLE_CATEGORIES . " c LEFT JOIN " . TABLE_CATEGORIES_DESCRIPTION . " cd on( c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' ) LEFT JOIN " . TABLE_CATEGORIES_DESCRIPTION . " cden on( c.categories_id = cden.categories_id and cden.language_id = 1 ) LEFT JOIN categories_amazon_description cad on (c.categories_amazon_id = cad.categories_id and cad.language_id = '" . (int)$languages_id . "') where c.parent_id = '" . (int)$current_category_id . "' and (c.parent_id or c.categories_id in (" . $admin_cat_access . ")) order by c.sort_order, categories_name");

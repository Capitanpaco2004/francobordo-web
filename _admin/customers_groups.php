...
*/
  ...
  require('includes/application_top.php');
    ...
    $cg_show_tax_array = [['id' => '1', 'text' => ENTRY_GROUP_SHOW_TAX_YES],
                              ['id' => '0', 'text' => ENTRY_GROUP_SHOW_TAX_NO]];
    $cg_tax_exempt_array = [['id' => '1', 'text' => ENTRY_GROUP_TAX_EXEMPT_YES],
                              ['id' => '0', 'text' => ENTRY_GROUP_TAX_EXEMPT_NO]];
	...
	if ($_POST['group_tax_rate_exempt_id'] && $_POST['group_tax_rate_exempt_settings'] == '1') {
      ...
      case 'deleteconfirm':
        ...
        while($customers_id = tep_db_fetch_array($customers_id_query)) {
            tep_db_query("UPDATE " . TABLE_CUSTOMERS . " set customers_group_id = '0' where customers_id='" . (int)$customers_id['customers_id'] . "'");
        }
        tep_redirect(tep_href_link(FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['cID', 'action'])));
        break;
 ...
 /*     echo '<pre>';
	...
	if ($_POST['group_tax_rate_exempt_id'] && $_POST['group_tax_rate_exempt_settings'] == '1') {
	   ...
	   $group_tax_rates_exempt = substr($group_tax_rates_exempt,0,strlen($group_tax_rates_exempt)-1);
        ...
        tep_db_query("insert into " . TABLE_CUSTOMERS_GROUPS . " set customers_group_id = '" . (int)$new_cg_id . "', customers_group_name = '" . tep_db_input($customers_group_name) . "', customers_group_show_tax = '" . $customers_group_show_tax . "', customers_group_tax_exempt = '" . $customers_group_tax_exempt . "', group_payment_allowed = '". $group_payment_allowed ."', group_shipment_allowed = '". $group_shipment_allowed ."', group_order_total_allowed = '". $group_order_total_allowed ."', group_specific_taxes_exempt = '". $group_tax_rates_exempt ."'");
...
<?php
  if ($_GET['action'] == 'edit') {
    ...
    $file_extension = substr((string) $_SERVER['PHP_SELF'], strrpos((string) $_SERVER['PHP_SELF'], '.'));
    $directory_array = [];
    ...
    $ship_directory_array = [];
    if ($dir = @dir($ship_module_directory)) {
     while ($file = $dir->read()) {
       if (!is_dir($ship_module_directory . $file) && substr($file, strrpos($file, '.')) === $file_extension) {
    ...
    $order_total_directory_array = [];
    if ($dir = @dir($order_total_module_directory)) {
     while ($file = $dir->read()) {
       if (!is_dir($order_total_module_directory . $file) && substr($file, strrpos($file, '.')) === $file_extension) {
           $order_total_directory_array[] = $file;
           // array of all order total modules present in includes/modules/order_total
       }
     }
...
function check_form() {
  ...
  if (customers_group_name == "") {
    error_message = "<?php
    echo ERROR_CUSTOMERS_GROUP_NAME;
    ?>";
    error = 1;
  }
...
//--></script>
	...
	<td>
	<?php
    echo tep_draw_form('customers', FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['action']) . 'action=update', 'post', 'onSubmit="return check_form();"');
    ?>
    ...
    ?></td>
      ...
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
          <tr>
            <td class="main"><?php
    echo ENTRY_GROUPS_NAME;
    ?></td>
            <td class="main"><?php
    echo tep_draw_input_field('customers_group_name', $cInfo->customers_group_name, 'maxlength="32"', false) . ENTRY_GROUP_NAME_MAX_LENGTH;
    ?></td>
          </tr>
          <tr>
            <td class="main"><?php
    echo ENTRY_GROUP_SHOW_TAX;
    ?></td>
            <td class="main"><?php
    echo tep_draw_pull_down_menu('customers_group_show_tax', $cg_show_tax_array, (($cInfo->customers_group_show_tax == '1') ? '1' : '0')) . ENTRY_GROUP_SHOW_TAX_EXPLAIN_1;
    ?></td>
          </tr>
          <tr>
            <td class="main">&#160;</td>
            <td class="main" style="line-height: 2"><?php
    echo ENTRY_GROUP_SHOW_TAX_EXPLAIN_2;
    ?></td>
          </tr>
          <tr>
            <td class="main"><?php
    echo ENTRY_GROUP_TAX_EXEMPT;
    ?></td>
            <td class="main"><?php
    echo tep_draw_pull_down_menu('customers_group_tax_exempt', $cg_tax_exempt_array, (($cInfo->customers_group_tax_exempt == '1') ? '1' : '0'));
    ?></td>
          </tr>
	</table>
	</td>
    ...
    ?></td>
      ...
      <tr>
        <td class="formAreaTitle"><?php
    include_once(DIR_WS_LANGUAGES . $language . '/modules.php');
    ...
    ?></td>
      ...
      <tr>
	   ...
	   <tr bgcolor="#DEE4E8">
            <td class="main"><?php
    echo tep_draw_radio_field('group_payment_settings', '1', false, (tep_not_null($cInfo->group_payment_allowed)? '1' : '0' )) . '&nbsp;&nbsp;' . ENTRY_GROUP_PAYMENT_SET . '&nbsp;&nbsp;' . tep_draw_radio_field('group_payment_settings', '0', false, (tep_not_null($cInfo->group_payment_allowed)? '1' : '0' )) . '&nbsp;&nbsp;' . ENTRY_GROUP_PAYMENT_DEFAULT ;
    ?></td>
          </tr>
<?php
...
<?php
    ...
    // end for ($i = 0, $n = count($directory_array); $i < $n; $i++)
    ?>
	   <tr>
            <td class="main" style="padding-left: 30px; padding-right: 10px; padding-top: 10px;"><?php
    echo ENTRY_PAYMENT_SET_EXPLAIN ?>
    ?></td>
           </tr>
        </table>
       </td>
    ...
    ?></td>
      ...
      <tr>
        <td class="formAreaTitle"><?php
    echo HEADING_TITLE_MODULES_SHIPPING;
    ?></td>
      </tr>
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
...
<?php
    ...
    for ($i = 0, $n = count($ship_directory_array); $i < $n; $i++) {
...
<?php
    ...
    // end for ($i = 0, $n = count($ship_directory_array); $i < $n; $i++)
    ?>
	   <tr>
            <td class="main" style="padding-left: 30px; padding-right: 10px; padding-top: 10px;"><?php
    echo ENTRY_SHIPPING_SET_EXPLAIN ?>
    ?></td>
           </tr>
        </table>
       </td>
    ...
    ?></td>
      ...
      <tr>
	   ...
	   <tr bgcolor="#DEE4E8">
            <td class="main"><?php
    echo tep_draw_radio_field('group_order_total_settings', '1', false, (tep_not_null($cInfo->group_order_total_allowed)? '1' : '0' )) . '&nbsp;&nbsp;' . ENTRY_GROUP_ORDER_TOTAL_SET . '&nbsp;&nbsp;' . tep_draw_radio_field('group_order_total_settings', '0', false, (tep_not_null($cInfo->group_order_total_allowed)? '1' : '0' )) . '&nbsp;&nbsp;' . ENTRY_GROUP_ORDER_TOTAL_DEFAULT ;
    ?></td>
          </tr>
<?php
    $order_total_module_active = explode (";",MODULE_ORDER_TOTAL_INSTALLED);
    $installed_order_total_modules = [];
    for ($i = 0, $n = count($order_total_directory_array); $i < $n; $i++) {
...
<?php
    ...
    // end for ($i = 0, $n = count($order_total_directory_array); $i < $n; $i++)
    ?>
	   <tr>
            <td class="main" style="padding-left: 30px; padding-right: 10px; padding-top: 10px;"><?php
    echo ENTRY_ORDER_TOTAL_SET_EXPLAIN ?>
    ?></td>
           </tr>
        </table>
       </td>
    ...
    ?></td>
      ...
      <tr>
        <td class="formAreaTitle"><?php
    echo HEADING_TITLE_GROUP_TAX_RATES_EXEMPT;
    ?></td>
      </tr>
      <tr>
        <td class="formArea"><table border="0" cellspacing="2" cellpadding="2">
	   <tr bgcolor="#DEE4E8">
            <td class="main"><?php
    echo tep_draw_radio_field('group_tax_rate_exempt_settings', '1', false, (tep_not_null($cInfo->group_specific_taxes_exempt)? '1' : '0' )) . '&nbsp;&nbsp;' . ENTRY_GROUP_TAX_RATES_EXEMPT . '&nbsp;&nbsp;' . tep_draw_radio_field('group_tax_rate_exempt_settings', '0', false, (tep_not_null($cInfo->group_specific_taxes_exempt)? '1' : '0' )) . '&nbsp;&nbsp;' . ENTRY_GROUP_TAX_RATES_DEFAULT ;
    ?></td>
           </tr>
<?php
    $tax_query = tep_db_query( 'select tr.tax_rates_id, tr.tax_rate, tr.tax_description, gz.geo_zone_name
									from ' . TABLE_TAX_RATES . ' tr
									LEFT JOIN ' . TABLE_TAX_CLASS . ' tc ON(tr.tax_class_id = tc.tax_class_id)
									LEFT JOIN ' . TABLE_GEO_ZONES . ' gz ON(tr.tax_zone_id = gz.geo_zone_id)
									order by tr.tax_rates_id');
    while ($tax_rate = tep_db_fetch_array($tax_query)) {
?>
	   <tr>
            <td class="main"><?php echo tep_draw_checkbox_field('group_tax_rate_exempt_id[' . $tax_rate['tax_rates_id'] . ']', $tax_rate['tax_rates_id'] , (in_array($tax_rate['tax_rates_id'], $group_tax_ids_exempt)) ? 1 : 0); ?>&#160;&#160;<?php echo $tax_rate['tax_description'] . ' (' . $tax_rate['geo_zone_name'] . ')'; ?></td>
           </tr>
<?php
}
    ...
    ?>
	   <tr>
            <td class="main" style="padding-left: 30px; padding-right: 10px; padding-top: 10px;"><?php
    echo ENTRY_TAX_RATES_EXEMPT_EXPLAIN ?>
    ?></td>
           </tr>
        </table>
       </td>
    ...
    echo tep_image_submit('button_update.png', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['action','cID'])) .'">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>';
    ?></td>
      </tr>
      </form>
	  ...
	  <tr>
        <td><?php
    echo tep_draw_separator('pixel_trans.png', '1', '70');
    ?></td>
      </tr>
</table>
</td>
...
//--></script>
    ...
    ?></td>
      ...
      <tr><?php
    echo tep_draw_form('customers', FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['action']) . 'action=newconfirm', 'post', 'onSubmit="return check_form();"');
    ?>
        <td class="formAreaTitle"><?php
    echo CATEGORY_PERSONAL;
    ?></td>
    ...
    ?></td>
            <td class="main"><?php
    echo tep_draw_input_field('customers_group_name', '', 'maxlength="32"', false);
    ?></td>
          </tr>
          <tr>
            <td class="main"><?php
    ...
    ?></td>
            <td class="main"><?php
    echo tep_draw_pull_down_menu('customers_group_show_tax', $cg_show_tax_array, '1') . ' ' . ENTRY_SHIPPING_SET_EXPLAIN_2;
    ?>  </td>
          </tr>
          <tr>
            <td class="main">&#160;</td>
            <td class="main" style="line-height: 2"></td>
          </tr>
          <tr>
            <td class="main"><?php
    echo ENTRY_GROUP_TAX_EXEMPT;
    ?></td>
            <td class="main"><?php
    echo tep_draw_pull_down_menu('customers_group_tax_exempt', $cg_tax_exempt_array, '0');
    ?></td>
          </tr>
	 </table>
	</td>
...
<?php
    ...
    ?></td>
      ...
      <tr>
	   ...
	   <tr bgcolor="#DEE4E8">
            <td class="main"><?php
    echo tep_draw_radio_field('group_payment_settings', '1', false, '0') . '&nbsp;&nbsp;' . ENTRY_GROUP_PAYMENT_SET . '&nbsp;&nbsp;' . tep_draw_radio_field('group_payment_settings', '0', false, '0') . '&nbsp;&nbsp;' . ENTRY_GROUP_PAYMENT_DEFAULT ;
    ?></td>
          </tr>
<?php
    $module_active = explode (";",MODULE_PAYMENT_INSTALLED);
    ...
    if (in_array ($directory_array[$i], $module_active)) {
     ...
     if (tep_class_exists($class)) {
       $module = new $class;
       if ($module->check() > 0) {
         $installed_modules[] = ['file_name' => $file, 'title' => $module->title];
       }
     } // end if (tep_class_exists($class))
   } // end if (in_array ($directory_array[$i], $module_active))
 }
    // end for ($i = 0, $n = count($directory_array); $i < $n; $i++)
    $counter = count($installed_modules);
    // end for ($i = 0, $n = count($directory_array); $i < $n; $i++)
    for ($y = 0; $y < $counter ; $y++) {
  ?>
  	   <tr>
  ...
  <?php
    ...
    // end for ($y = 0; $y < count($installed_modules) ; $y++)
    ?>
	   <tr>
            <td class="main" style="padding-left: 30px; padding-right: 10px; padding-top: 10px;"><?php
    echo ENTRY_PAYMENT_SET_EXPLAIN ?>
...
<?php
    ...
    if (in_array ($ship_directory_array[$i], $ship_module_active)) {
     ...
     if (tep_class_exists($ship_class)) {
       $ship_module = new $ship_class;
       if ($ship_module->check() > 0) {
         $installed_shipping_modules[] = ['file_name' => $file, 'title' => $ship_module->title];
       }
     } // end if (tep_class_exists($ship_class))
   } // end if (in_array ($ship_directory_array[$i], $ship_module_active))
 }
    // end for ($i = 0, $n = count($ship_directory_array); $i < $n; $i++)
    $counter = count($installed_shipping_modules);
    // end for ($i = 0, $n = count($ship_directory_array); $i < $n; $i++)
    for ($y = 0; $y < $counter ; $y++) {
   ?>
   	   <tr>
   ...
   <?php
    ...
    // end for ($y = 0; $y < count($installed_shipping_modules) ; $y++)
    ?>
	   <tr>
            <td class="main" style="padding-left: 30px; padding-right: 10px; padding-top: 10px;"><?php
    echo ENTRY_SHIPPING_SET_EXPLAIN ?>
...
<?php
    ...
    if (in_array ($order_total_directory_array[$i], $order_total_module_active)) {
     ...
     if (tep_class_exists($order_total_class)) {
       $order_total_module = new $order_total_class;
       if ($order_total_module->check() > 0) {
         $installed_order_total_modules[] = ['file_name' => $file, 'title' => $order_total_module->title];
       }
     } // end if (tep_class_exists($order_total_class))
   } // end if (in_array ($order_total_directory_array[$i], $order_total_module_active))
 }
    // end for ($i = 0, $n = count($order_total_directory_array); $i < $n; $i++)
    $counter = count($installed_order_total_modules);
    // end for ($i = 0, $n = count($order_total_directory_array); $i < $n; $i++)
    for ($y = 0; $y < $counter ; $y++) {
   ?>
   	   <tr>
   ...
   <?php
    ...
    // end for ($y = 0; $y < count($installed_order_total_modules) ; $y++)
    ?>
	   <tr>
            <td class="main" style="padding-left: 30px; padding-right: 10px; padding-top: 10px;"><?php
    echo ENTRY_ORDER_TOTAL_SET_EXPLAIN ?>
    ...
    ?></td>
      ...
      <tr>
	   ...
	   <tr bgcolor="#DEE4E8">
            <td class="main"><?php
    echo tep_draw_radio_field('group_tax_rate_exempt_settings', '1', false, '0') . '&nbsp;&nbsp;' . ENTRY_GROUP_TAX_RATES_EXEMPT . '&nbsp;&nbsp;' . tep_draw_radio_field('group_tax_rate_exempt_settings', '0', false, '0') . '&nbsp;&nbsp;' . ENTRY_GROUP_TAX_RATES_DEFAULT ;
    ?></td>
           </tr>
<?php
    $tax_query = tep_db_query( 'select tr.tax_rates_id, tr.tax_rate, tr.tax_description, gz.geo_zone_name
...
?>
	   <tr>
            <td class="main"><?php echo tep_draw_checkbox_field('group_tax_rate_exempt_id[' . $tax_rate['tax_rates_id'] . ']', $tax_rate['tax_rates_id'] , 0); ?>&#160;&#160;<?php echo $tax_rate['tax_description'] . ' (' . $tax_rate['geo_zone_name'] . ')'; ?></td>
           </tr>
<?php
}
    // end while ($tax_rate = tep_fetch_array($tax_query)
...
?>
      ...
      <tr>
          ...
          <?php
          $order = match ($_GET['listing']) {
    "group" => "g.customers_group_name",
    "group-desc" => "g.customers_group_name DESC",
    default => "g.customers_group_id ASC",
};
        ?>
	    <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
        <tr>
          <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
	       ...
	       <td class="dataTableHeadingContent"><a href="<?php echo $_SERVER['PHP_SELF'] . '?listing=group'; ?>"><?php echo tep_image_button('ic_up.gif', ' ' . ENTRY_GROUP_TEXT_SORT . ' ' . TABLE_HEADING_NAME . ' --> A-B-C ' . ENTRY_GROUP_TEXT_FROM_TOP . ' '); ?></a>&nbsp;<a href="<?php echo $_SERVER['PHP_SELF'] . '?listing=group-desc'; ?>"><?php echo tep_image_button('ic_down.gif', ' ' . ENTRY_GROUP_TEXT_SORT . ' ' . TABLE_HEADING_NAME . ' --> Z-X-Y ' . ENTRY_GROUP_TEXT_FROM_TOP . ' '); ?></a>&nbsp;<?php echo TABLE_HEADING_NAME; ?></td>
...
<?php
  ...
  while ($customers_groups = tep_db_fetch_array($customers_groups_query)) {
    ...
    if ( (is_object($cInfo)) && ($customers_groups['customers_group_id'] == $cInfo->customers_group_id) ) {
      echo '          <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['cID', 'action']) . 'cID=' . $cInfo->customers_group_id . '&action=edit') . '\'">' . "\n";
    } else {
      echo '          <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['cID']) . 'cID=' . $customers_groups['customers_group_id']) . '\'">' . "\n";
    }
?>
                <td class="dataTableContent"><?php echo $customers_groups['customers_group_name']; ?></td>
                <td class="dataTableContent" align="right"><?php if ( (is_object($cInfo)) && ($customers_groups['customers_group_id'] == $cInfo->customers_group_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.png', ''); } else { echo '<a href="' . tep_href_link(FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['cID']) . 'cID=' . $customers_groups['customers_group_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
              <tr>
                <td colspan="4"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td class="smallText" align="right"><?php echo $customers_groups_split->display_links($customers_groups_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $_GET['page'], tep_get_all_get_params(['page', 'info', 'x', 'y', 'cID'])); ?></td>
                  </tr>
<?php
...
switch ($_GET['action']) {
  case 'confirm':
      if ($_GET['cID'] != '0') {
          $heading[] = ['text' => ''. tep_draw_separator('pixel_trans.png', '11', '12') .'&nbsp;<br><b>' . TEXT_INFO_HEADING_DELETE_GROUP . '</b>'];
          $contents = ['form' => tep_draw_form('customers_groups', FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['cID', 'action']) . 'cID=' . $cInfo->customers_group_id . '&action=deleteconfirm')];
          $contents[] = ['text' => TEXT_DELETE_INTRO . '<br><br><b>' . $cInfo->customers_group_name . ' </b>'];
          ...
          $contents[] = ['align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['cID', 'action']) . 'cID=' . $cInfo->customers_group_id) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>'];
      } else {
          $heading[] = ['text' => ''. tep_draw_separator('pixel_trans.png', '11', '12') .'&nbsp;<br><b>' . TEXT_INFO_HEADING_DELETE_GROUP . '</b>'];
          $contents[] = ['text' => ENTRY_GROUP_DELETE_CONFIRM_NOT_ALLOWED . '<br><br><b>' . $cInfo->customers_group_name . ' </b>'];
      }
    break;
  default:
    if (is_object($cInfo)) {
      $heading[] = ['text' => ''. tep_draw_separator('pixel_trans.png', '11', '12') .'&nbsp;<b>' . $cInfo->customers_group_name . '</b>'];
      $contents[] = ['align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['cID', 'action']) . 'cID=' . $cInfo->customers_group_id . '&action=edit') . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_CUSTOMERS_GROUPS, tep_get_all_get_params(['cID', 'action']) . 'cID=' . $cInfo->customers_group_id . '&action=confirm') . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a>'];

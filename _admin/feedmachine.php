...
if (!defined('DIR_WS_HTTPS_CATALOG')) define('DIR_WS_HTTPS_CATALOG', '/');
...
function field_generator($db_field_name)
{
    ...
    global $product, $current_product_prices, $cur_feed, $currencies_info, $countries, $categories;

    switch ($db_field_name) {
        case 'PRODUCTS_URL':
            $additional_params = array();
            if ($cur_feed['currency_code'] != DEFAULT_CURRENCY) {
                $additional_params[] = 'currency=' . $cur_feed['currency_code'];
            }

            if ($cur_feed['language_code'] != DEFAULT_LANGUAGE) {
                $additional_params[] = 'language=' . $cur_feed['language_code'];
            }

            if (!empty($cur_feed['url_parameters'])) {
                $additional_params[] = $cur_feed['url_parameters'];
            }
             ...
             */
            if ((int) $product['products_options_values_id'] > 0) {
                $additional_params[] = 'attr=' . $product['products_options_values_id'];
            }

            $additional_params_string = !empty($additional_params) ? '&' . implode('&', $additional_params) : '';
            if (strpos($product['products_id'], '-') !== false) {
                $url = strstr($product['products_id'], '-', true);
            } else {
                $url = $product['products_id'];
            }
            $output_field_value = tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $url . $additional_params_string, 'NONSSL', false);
            break;
        case 'IMAGE_URL':
            $output_field_value = $product['products_image'] ? HTTP_SERVER . '/' . DIR_WS_IMAGES . 'productos/' . $product['products_image'] : '';
            break;
        case 'CATEGORY':
        ...
        case 'UNIQUE_ID':
            ...
            break;
        case 'IS_IN_STOCK':
            $output_field_value = $product['products_quantity'] > -900 ? 'in stock' : 'out of stock';
            break;
        case 'BLANK':
        default:
            $output_field_value = '';
...
function category_path($categories_id, &$language_id, &$cat_path)
{
    ...
    $cat_path = array_merge(array(
        ($cat_info["categories_name"] ?? ""),
    ), $cat_path);

    if (($cat_info["parent_id"] ?? false)) {
...
function combination(&$options, $options_seperator = '|', $pos = 0, $combinations = false)
{
    ...
    if (++$pos < sizeof($options)) {
        return combination($options, $options_seperator, $pos, $new_combinations);
    } else {
...
$currencies_info_query = tep_db_query('SELECT * FROM ' . TABLE_CURRENCIES);
while ($currencies_info_row = tep_db_fetch_array($currencies_info_query)) {
    $currencies_info[$currencies_info_row['code']] = $currencies_info_row;
}
tep_db_free_result($currencies_info_query);
...
foreach ($feeds as $cur_feed_id => $cur_feed) {
    $feeds[$cur_feed_id]['currency_decimal'] = $feeds[$cur_feed_id]['currency_decimal_override'] !== false ? $feeds[$cur_feed_id]['currency_decimal_override'] : $currencies_info[$cur_feed['currency_code']]['decimal_point'];
    $feeds[$cur_feed_id]['currency_thousands'] = $feeds[$cur_feed_id]['currency_thousands_override'] !== false ? $feeds[$cur_feed_id]['currency_thousands_override'] : $currencies_info[$cur_feed['currency_code']]['thousands_point'];
}
//
...
while (true) {
    ...
    $sql = 'SELECT ' .
        ...
        'ps.products_stock_quantity, m.manufacturers_name, p.products_model, pd.products_description, p.products_cost, s.*, cd.*, c.*, ptc.*, mi.*, m.*, pd.*, p.*, ps.*,
IF (pa.products_attributes_id IS NOT NULL, CONCAT(p.products_id,"-",pa.products_attributes_id), p.products_id) as products_id,
IF (pov.products_options_values_name IS NOT NULL, CONCAT(pd.products_name," - ",pov.products_options_values_name), pd.products_name) as products_name,
...
IF (ps.products_stock_cost > 0, ps.products_stock_cost, p.products_cost) as products_cost,
(CASE
...
END) as final_price
     ...
     LEFT JOIN products_options_values pov ON (pa.options_values_id = pov.products_options_values_id AND pov.language_id IN(' . $languages_list . '))
     LEFT JOIN products_stock ps ON p.products_id = ps.products_id AND ps.products_stock_attributes = CONCAT(pa.options_id, "-", pa.options_values_id)
    ...
    if ($begin_output_run) {
        ...
        foreach ($feeds as $cur_feed_id => $cur_feed) {
            ...
            if (file_exists($file)) {
                if (defined('FM_BACK_UP_OLD_FEEDS') && FM_BACK_UP_OLD_FEEDS) {
                    ...
                    $file_info = array();
                    if (preg_match('#^(.*?)(\..*?)?$#', $feeds[$cur_feed_id]['filename'], $file_info)) {
                        rename($file, $save_path . $file_info[1] . '-' . $old_file_last_modified . ($file_info[2] ?? ''));
                    }
                } else {
                    unlink($file);
                }
        ...
        while ($cur_field = tep_db_fetch_array($master_query)) {
            foreach ($cur_field as $key => $value) {
                $fields[] = $key;
            }
            break;
        }
    ...
    while ($master_query_row = tep_db_fetch_array($master_query)) {
        if ($products_id_low == 0) {
            $products_id_low = $master_query_row['products_id'];
        }
    ...
    if (!defined('PERFORMANCE_SKIP_LOAD_ATTRIBUTES') || (defined('PERFORMANCE_SKIP_LOAD_ATTRIBUTES') && !PERFORMANCE_SKIP_LOAD_ATTRIBUTES)) {
        ...
        if (tep_db_num_rows($products_attributes_query) > 0) {
            while ($products_attributes_row = tep_db_fetch_array($products_attributes_query)) {
                if (isset($products[(string) $products_attributes_row['language_id']][$products_attributes_row['products_id']])) {
            ...
            if ($i_rows_required) {
                foreach ($products_attributes as $language_id => $value) {
                    foreach ($value as $products_id => $options_values) {
                        ...
                        $combinations = combination($options, $options_seperator);
    ...
    ob_flush();
    foreach ($feeds as $cur_feed_id => $cur_feed) {
        if (!empty($products[$cur_feed['language_id']])) {
            foreach ($products[$cur_feed['language_id']] as $row) {
                ...
                foreach ($rows as $product) {
                    ...
                    if (!empty($cur_feed['include_record_function'])) {
                        $include_record = @call_user_func_array($cur_feed['include_record_function'], array(
                            &$product,
                        ));
                        if (!$include_record) {
                            continue;
                    ...
                    $currency_conversion_field_list = array(
                        'products_price',
                        'final_price',
                    ...
                    foreach ($currency_conversion_field_list as $field_name) {
                        $current_product_prices[$field_name . '_raw'] = isset($product[$field_name]) ? ($product[$field_name] * (isset($cur_feed['currency_code']) ? $currencies_info[$cur_feed['currency_code']]['value'] : 1)) : '';

                        $current_product_prices[$field_name] = !empty($current_product_prices[$field_name . '_raw']) ? number_format(tep_add_tax($current_product_prices[$field_name . '_raw'], ($taxes[$cur_feed['countries_iso_2']][$product['products_tax_class_id']] ?? 0)), $currencies_info[$cur_feed['currency_code']]['decimal_places'], $cur_feed['currency_decimal'], $cur_feed['currency_thousands']) : '';
                        $current_product_prices[$field_name . '_without_tax'] = !empty($current_product_prices[$field_name . '_raw']) ? number_format($current_product_prices[$field_name . '_raw'], $currencies_info[$cur_feed['currency_code']]['decimal_places'], $cur_feed['currency_decimal'], $cur_feed['currency_thousands']) : '';
                        $current_product_prices[$field_name . '_with_tax'] = !empty($current_product_prices[$field_name . '_raw']) ? number_format($tax_multiplier * $current_product_prices[$field_name . '_raw'], $currencies_info[$cur_feed['currency_code']]['decimal_places'], $cur_feed['currency_decimal'], $cur_feed['currency_thousands']) : '';
                    }
                    ...
                    foreach ($cur_feed['fields'] as $output_field_name => $db_field) {
                        ...
                        switch ($db_field['type']) {
                            case 'DB':
                                $output_field_value = substr((string)$product[$db_field['output']], 0, 5000);
                                break;
                            case 'KEYWORD':
                        ...
                        if (isset($db_field['options'])) {
                            $output_field_value = process_field_value($output_field_value, $db_field['options']);
                        }
                    ...
                    if (!empty($feeds[$cur_feed_id]['encoding']) && $feeds[$cur_feed_id]['encoding'] !== false) {
                        switch ($feeds[$cur_feed_id]['encoding']) {
                            case 'utf8':
                                $output_line = mb_convert_encoding($output_line ?? '', 'UTF-8', 'ISO-8859-1');
                                break;
                        }
                    }
...
foreach ($fps as $cur_feed_id => $fp) {
    ...
    if (isset($feeds[$cur_feed_id]['compression_compress']) && $feeds[$cur_feed_id]['compression_compress']) {
        ...
        if (empty($feeds[$cur_feed_id]['compression_type'])) {
            $feeds[$cur_feed_id]['compression_type'] = 'gzip';
        }

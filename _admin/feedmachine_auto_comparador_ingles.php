...
// Procesar SOLO la config "comparador_ingles.php" — no afectar al resto de feeds
$feeds = isset($feeds['comparador_ingles.php']) ? array('comparador_ingles.php' => $feeds['comparador_ingles.php']) : array();
...
foreach( $feeds as $feed_id => $feed ) {
  if( empty($feeds[$feed_id]['name']) )
    $feeds[$feed_id]['name'] = $feed['filename'];
...
/* @daniel.lucia: revisamos si solo se deben hacer feed con atributos y reorganizamos feeds */
$feeds_alt = array();
foreach( $feeds as $feed_id => $feed ) {
    if ((int)$_GET['with_attributes'] == 1) {
        if ((int)($feed['include_attributes'] ?? 0) == 1) {
            $feeds_alt[] = $feed;
        }
    } else {
        if ((int)($feed['include_attributes'] ?? 0) == 0) {
            $feeds_alt[] = $feed;
        }
    }
}
...
//run checks — fichero separado para no machacar el tracking de los otros feeds
$fmu = FM_SAVE_LOCATION . 'fm-info-comparador-ingles.txt';
if( !$force_update && file_exists($fmu) ) {
  ...
  foreach( $update_file as $info ) {
	...
	$update_info_feeds[$temp[0]]['configuration_hash'] = $temp[2] ?? '';
	$update_info_feeds[$temp[0]]['last_generation'] = $temp[3] ?? '';
  }
  ...
  if( !$global_update ) {
	foreach( $feeds as $feed_id => $feed ) {
	  ...
	  if( isset($feed['force_regeneration_period']) && isset($update_info_feeds[$feed['name']]['last_generation']) && $update_info_feeds[$feed['name']]['last_generation'] <= (time()-$feed['force_regeneration_period']*86400) ) {
		$generate[$feed_id] = true;
		$updated = true;
		continue;
	  }
...
// Generador clonado sin filtro exclude_feedmachine
require($installation_path . 'feedmachine_with_attributes_comparador.php');
  ...
  foreach( $feeds as $feed_id => $feed ) {
	...
	if( isset($update_info_feeds[$feed['name']]) && $update_info_feeds[$feed['name']]['last_upload'] < $catalog_last_updated ) {
	  $ftp_feeds = true;
	  $ftp_send[$feed_id] = true;
	  $fm_auto_report['feeds'][ $feed['name'] ]['upload_trigger_status'] = 'feed upload triggered because ftp failed previously';
	  continue;
	}
	if( isset($feed['ftp_upload_period']) && isset($update_info_feeds[$feed['name']]) && $update_info_feeds[$feed['name']]['last_upload'] <= (time()-$feed['ftp_upload_period']*86400) ) {
	  $ftp_feeds = true;
	  $ftp_send[$feed_id] = true;

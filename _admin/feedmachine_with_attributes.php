...
function field_generator($db_field_name)
{
    ...
    switch ($db_field_name) {
        case 'PRODUCTS_URL':
            ...
            $output_field_value = tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $url . $additional_params_string . '&language=es', 'NONSSL', false);
...
while (true) {
    ...
    $sql = 'SELECT ' .
        ...
        'ps.products_stock_quantity, m.manufacturers_name, p.products_model, pd.products_description, p.products_cost, s.*, cd.*, c.*, ptc.*, mi.*, m.*, pd.*, p.*,
...
END) as final_price
     ...
     p.exclude_feedmachine = 0 AND
     (p.product_ean <>"" OR (pa.products_attributes_ean IS NOT NULL AND pa.products_attributes_ean <>"" AND pa.products_attributes_ean <>"0"))

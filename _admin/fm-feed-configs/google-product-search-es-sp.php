...
$feed_config = array('name' => 'Google Product Search',
                     'authors' => 'raiwa',
                     'filename' => 'francobordo_antiguo.txt', //change the name and the filename to a unique name for each language and country
                     'schema_version' => '2.0',
                     'fields' => array('id'               =>   array('output' => 'products_id',
                                       ...
                                       'price'            =>   array('output' => 'FM_RS_final_price_with_TAX_es_sp',
                                                                     'type' => 'FUNCTION'
                                                                    ),
		                      ...
		                      'gtin'              =>   array('output' => 'product_ean',
                                                                     'type' => 'DB'
                                                                    ),				    
                         'google_product_category'        =>   array('output' => 'FM_RS_google_categories_es_sp',  //change the name to the name used for the function
                                                                     'type' => 'FUNCTION'
                                                                    ),
                                       'product_type'     =>   array('output' => 'CATEGORY_TREE',
                                                                     'type' => 'KEYWORD',
                                                                     'options' => array('STRIP_HTML', 'STRIP_CRLF'),
                                                                     'filters' => array('patterns' => array('#Submarinismo-Fusiles y Cuchillos-#'),
                                                                     	            'replacements' => array('Submarinismo-Cuchillos-'))
                                                                     ),
...
function FM_RS_product_id_es_sp($product) {
  return 'Francobordo' . $product['products_id'] . '_es_sp';
}

function FM_RS_google_categories_es_sp($product) {
	...
	return  (($output_field_category == 2) ? 'Equipamiento deportivo > Deportes acuáticos > Deportes acuáticos remolcados > lanchas hinchables y botes remolcables':
		...
		(($output_field_category == 28) ? 'ehículos y recambios > Piezas y accesorios para vehículos > Accesorios y piezas para embarcaciones > Atracada y anclaje > Anclas':
...
function FM_RS_final_price_with_tax_es_sp($product) 
{

...
const KENT_CSV          = '/home/francobordo/public_html/import/feed/KENT/KENT_products_francobordo.csv';
const KENT_SFTP_HOST    = 'stftpamgprd01.blob.core.windows.net'; // SFTP Alliance Marine (Azure Blob)
const KENT_SFTP_PORT    = 22;
const KENT_SFTP_USER    = 'stftpamgprd01.amghubfrancobordo';
const KENT_SFTP_PASS    = 'lkYgzugus0nD0bQi/smYJLzSMY2RyFBS';
const KENT_SFTP_REMOTE  = 'products/KENT_products_francobordo.csv';
define('IMG_ABS_DIR',     dirname(dirname(__FILE__)) . '/images/productos/');
const NEW_CATEGORY_ID   = 1968; // "KENT Nuevos" (parent 0, status 0, creada 2026-07-07)
...
function kentTranslateLabel($label, $to) {
	$out = (string) $label;
...
function kentLabelNeedsLlm($label) {
...
function kentTranslateLabelFull($label, $to, $useLlm) {
	$dict = kentTranslateLabel($label, $to);
	if (!$useLlm || !kentLabelNeedsLlm($label)) return $dict;
	if (!isset($GLOBALS['kentLabelLlmCache'])) $GLOBALS['kentLabelLlmCache'] = [];
	$key = $to . '|' . $label;
	if (isset($GLOBALS['kentLabelLlmCache'][$key])) return $GLOBALS['kentLabelLlmCache'][$key];
	$out = llmTranslate($label, 2, max(24, mb_strlen($label, 'UTF-8') * 3), $to === 0 ? LLM_LABEL_PROMPT_ES : LLM_LABEL_PROMPT_EN);
	$res = $out !== '' ? $out : $dict;
	$GLOBALS['kentLabelLlmCache'][$key] = $res;
	return $res;
}
...
const UNIVERSE_ES = [
	'POMPE & EAU À BORD'    => 'Bombas y Agua a Bordo',
	'PASSERELLE & PONT'     => 'Pasarela y Cubierta',
	'ELECTRICITÉ'           => 'Electricidad',
	'ELECTRONIQUE'          => 'Electrónica',
	'ENVIRONNEMENT MOTEUR'  => 'Entorno del Motor',
	'CONFORT À BORD'        => 'Confort a Bordo',
	'COMMANDE & DIRECTION'  => 'Mandos y Dirección',
	'ECLAIRAGE'             => 'Iluminación',
	'PROPULSEUR'            => 'Hélices de Maniobra',
	'SIGNALISATION'         => 'Señalización',
	'ANODE'                 => 'Ánodos',
	'STABILISATION'         => 'Estabilización',
	'MOUILLAGE ET ACCASTILLAGE' => 'Fondeo y Accesorios',
	'ARCHIVES'              => 'Archivo',
	'PERFORMANCES ET STABILISATION' => 'Estabilización',
	'ANODES ET CONSOMMABLES D\'ATELIER' => 'Ánodos',
];
...
/** Parsea números del feed KENT (decimal con punto; tolera coma europea). Null si no numérico. */
function kentParseNum($v) {
	$v = trim((string) $v);
	if ($v === '') return null;
...
function kentFlattenSpecTables($html) {
...
function kentExtractSpecs($html) {
	$specs = [];
	$html = (string) $html;
...
function kentBuildFamilySpecTable(array $items, array $labels, $langId, $useLlm) {
	$isEs = ($langId === LANG_ID_ES);
	$specsBySku = [];
	$specCols = [];
	foreach ($items as $sku => $it) {
		$src = (!$isEs && $it['LONG_EN'] !== '') ? $it['LONG_EN'] : $it['LONG_FR'];
		$sp = kentExtractSpecs($src);
		$specsBySku[$sku] = $sp;
	...
	$headTr = [];
	foreach ($specCols as $k) $headTr[$k] = kentTranslateLabelFull($k, $isEs ? 0 : 1, $useLlm);
	...
	$html = '<br><br><!--KENT_SPECS_START--><p><strong>' . htmlspecialchars($title) . '</strong></p>' . $open . '<tr>';
	...
	$html .= '</tbody></table><!--KENT_SPECS_END-->';
	return $html;
}

function cleanHtmlAggressive($html) {
	if ($html === null || $html === '') return '';
	$html = kentFlattenSpecTables($html);
...
function kentSlugify($text, $maxLen = 50) {
	$t = trim((string) $text);
...
function kentBrandPrefix($rawBrand, $rawName = '') {
...
function resolveManufacturer($mysqli, $rawName, &$cache, &$createdLog, $dryRun) {
	...
	$display = kentNormalizeManufacturer($rawName);
...
/** Subcategoría de staging por Universo bajo KENT Nuevos (status=0, oculta), con nombre ES+EN.
...
/** Mapa de duplicados — FILTRADO por origin KENT (multi-marca, como FNI/Lankhorst).
...
function kentExtractMeasure($title) {
	$title = (string) $title;
...
/** Descarga KENT_products_francobordo.csv vía SFTP (Azure Blob). Devuelve ['ok'=>bool,...]. */
function downloadFeedFromSftp($localPath) {
	try {
		$dir = dirname($localPath);
		if (!is_dir($dir)) @mkdir($dir, 0775, true);
		$sftp = new SFTP(KENT_SFTP_HOST, KENT_SFTP_PORT, 30);
		if (!$sftp->login(KENT_SFTP_USER, KENT_SFTP_PASS)) return ['ok' => false, 'err' => 'login fallido'];
		$tmp = $localPath . '.tmp.' . uniqid();
		if (!$sftp->get(KENT_SFTP_REMOTE, $tmp)) return ['ok' => false, 'err' => 'get fallido'];
...
/** Lee el CSV KENT (pipe |, UTF-8, CON cabecera) mapeando por nombre de columna.
...
function loadCsvRows($path, &$statusSkipped = 0, &$badRows = 0) {
	...
	while (($r = fgetcsv($f, 0, '|', chr(34), '')) !== false) {
		...
		$rows[] = [
			...
			'EAN'        => kentNormalizeEan($col($r, 'Barcode')),
...
function kentDownloadImages($mysqli, $pid, array $row, $nameForSlug) {
	$urls = [];
...
<?php if ($isAction): ?>
	<h2>Importador KENT (Alliance Marine) — <?php echo $dryRun ? 'DRY-RUN (sin cambios)' : 'EJECUCIÓN REAL'; ?></h2>
	<p>
		<a href="<?php echo tep_href_link('import-kent-altas.php'); ?>" class="xbutton small hv9">← Volver</a>
	</p>
...
if (NEW_CATEGORY_ID <= 0) { logMsg("ERROR: NEW_CATEGORY_ID sin fijar (crear categoría 'KENT Nuevos' y poner su id)"); goto end_action; }

if (!$skipDownload) {
	$mtime = file_exists(KENT_CSV) ? filemtime(KENT_CSV) : 0;
	logMsg("Descargando " . KENT_SFTP_REMOTE . " desde SFTP " . KENT_SFTP_HOST . " …" . ($mtime ? " (local actual: " . date('Y-m-d H:i', $mtime) . ", " . round(filesize(KENT_CSV)/1048576, 1) . " MB)" : ""));
	$dl = downloadFeedFromSftp(KENT_CSV);
	if ($dl['ok']) {
		logMsg("  ✓ descargado: " . round($dl['size']/1048576, 1) . " MB (mtime " . date('Y-m-d H:i', filemtime(KENT_CSV)) . ")");
	} else {
		logMsg("  ✗ descarga fallida: " . $dl['err'] . " — uso copia local existente si la hay");
		if (!file_exists(KENT_CSV)) { logMsg("ERROR: no hay copia local y SFTP falló"); goto end_action; }
	}
}

if (!file_exists(KENT_CSV)) { logMsg("ERROR: CSV no encontrado: " . KENT_CSV); goto end_action; }
logMsg("Leyendo CSV…");
$statusSkipped = 0; $badRows = 0;
$rows = loadCsvRows(KENT_CSV, $statusSkipped, $badRows);
...
logMsg("  → " . count($existing) . " referencias ya en BD (scope origin=kent + EAN global + lista negra)");
...
foreach ($rows as $row) {
	...
	// Dedup: SKU KENT, referencia de fabricante y EAN
	...
	$net = kentParseNum($row['NET_PRICE']);
	if ($net === null || $net <= 0) { $skippedBadPrice++; continue; }
	$pub = kentParseNum($row['PUBLIC']);
	$cost = $net;
...
function kentBuildTexts(array $row, $brandPrefix, $skipTranslation, $dryRun, &$translateFail) {
	$rawNameFr = $row['NAME_FR'] !== '' ? $row['NAME_FR'] : $row['NAME_EN'];
	$rawNameEn = $row['NAME_EN'] !== '' ? $row['NAME_EN'] : $row['NAME_FR'];
	$titleFr = kentApplyBrandPrefix($brandPrefix, $rawNameFr);
	$titleEn = kentApplyBrandPrefix($brandPrefix, $rawNameEn);
	...
	if (!$skipTranslation && !$dryRun) {
		...
		if ($tn !== '') $nameEs = kentApplyBrandPrefix($brandPrefix, $tn); else $translateFail++;
...
foreach ($families as $parent => $items) {
	...
	$brandPrefix = kentBrandPrefix($brand, $cheap['NAME_FR'] !== '' ? $cheap['NAME_FR'] : $cheap['NAME_EN']);
	...
	list($nameEs, $descEs, $titleEn, $descEn) = kentBuildTexts($cheapForText, $brandPrefix, $skipTranslation, $dryRun, $translateFail);
	...
	foreach ($items as $sku => $it) {
		$labelCandidates[$sku] = [
			...
			'measure_fr' => kentExtractMeasure($frTitlesAll[$sku]),
			'measure_en' => kentExtractMeasure($enTitlesAll[$sku]),
		];
	}
	...
	foreach ($items as $sku => $it) {
		...
		$be = $baseEn[$sku] ?? '';
		if ($be === '') $be = $bf;
		$labelsEs[$sku] = mb_substr(kentTranslateLabelFull($bf, 0, $useLlmLabels), 0, 64, 'UTF-8');
		$labelsEn[$sku] = mb_substr(kentTranslateLabelFull($be, 1, $useLlmLabels), 0, 64, 'UTF-8');
	}

	// Tabla "Variantes y especificaciones" al final de la descripción (solo familias)
	if (!$dryRun) {
		$descEs .= kentBuildFamilySpecTable($items, $labelsEs, LANG_ID_ES, $useLlmLabels);
		$descEn .= kentBuildFamilySpecTable($items, $labelsEn, LANG_ID_EN, $useLlmLabels);
	}
	...
	try {
		// model = referencia del fabricante si la hay (mejor dedup/visible); reference_prov = SKU KENT (código de pedido)
		...
		list($imgOk, $subs) = kentDownloadImages($mysqli, $pid, $cheap, $nameEs ?: $titleEn);
...
foreach ($standalone as $sku => $row) {
	...
	$brandPrefix = kentBrandPrefix($row['BRAND'], $row['NAME_FR'] !== '' ? $row['NAME_FR'] : $row['NAME_EN']);
	$catId = getOrCreateSubcategory($mysqli, $row['_UNIV_ES'], $row['_UNIV_EN'], NEW_CATEGORY_ID, $dryRun, $subcatCache, $subcatCreated);

	list($nameEs, $descEs, $titleEn, $descEn) = kentBuildTexts($row, $brandPrefix, $skipTranslation, $dryRun, $translateFail);
	...
	try {
		...
		list($imgOk, $subs) = kentDownloadImages($mysqli, $pid, $row, $nameEs ?: $titleEn);
		if ($imgOk) { $nWithImg++; $nSubImgs += $subs; }
...
<?php else: ?>
	<h2>Importador KENT / Alliance Marine (altas) — variantes por Parent_id</h2>
	<p>
		Descarga <code><?php echo KENT_SFTP_REMOTE; ?></code> del SFTP de Alliance Marine (Azure), agrupa SKUs por
		...
		<strong>KENT Nuevos</strong> (cat <?php echo NEW_CATEGORY_ID; ?>) en
	...
	<form method="get" style="background:#f5f5f5;padding:15px;border-radius:5px;">
		<p>
			...
			<select name="universe" style="min-width:260px;">
				...
				<?php
				$univList = file_exists(KENT_CSV) ? listUniversesFromCsv(KENT_CSV) : [];
			...
			<select name="brand" style="min-width:300px;">
				...
				<?php
				$brandList = file_exists(KENT_CSV) ? listBrandsFromCsv(KENT_CSV) : [];
			...
			<?php if (!file_exists(KENT_CSV)): ?>
				<em style="color:#a00;">— CSV no descargado todavía; ejecuta una vez para popular los desplegables.</em>
			<?php endif; ?>
		</p>
		<p>
			<strong>SKUs específicos</strong> (opcional; acepta SKU KENT o referencia de fabricante; ignora filtros y máximo; si es variante importa toda la familia):<br>
	...
	<p style="margin-top:20px;color:#888;font-size:12px;">
		...
		- <code>products_model</code> = referencia del fabricante (supplier_item_code); <code>reference_prov</code> = SKU KENT (código de pedido).<br>
		...
		- <strong>Familias: tabla "Variantes y especificaciones"</strong> al final de la descripción (Referencia | Modelo | specs del feed | Peso si difiere), estilo tabla Osculati, entre marcadores KENT_SPECS. Si el cuerpo era solo la tabla de specs del feed, se omite (la tabla de familia ya lo cubre).<br>

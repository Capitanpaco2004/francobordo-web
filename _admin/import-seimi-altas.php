...
foreach ($families as $parent => $items) {
	...
	list($nameEs, $descEs, $titleEn, $descEn) = seimiBuildTexts($cheap, $brandPrefix, $skipTranslation, $dryRun, $translateFail);
	...
	try {
		...
		foreach ($items as $sku => $it) {
			...
			$be = $baseEn[$sku] ?? '';
			if ($be === '') $be = $bf;
			$labelsEs[$sku] = mb_substr(seimiTranslateLabel($bf, 0), 0, 64, 'UTF-8');
			$labelsEn[$sku] = mb_substr(seimiTranslateLabel($be, 1), 0, 64, 'UTF-8');
		}
...
<?php else: ?>
	...
	<p>
		...
		e inserta lo que no exista en BD bajo <strong>SEIMI Nuevos</strong> (cat <?php echo NEW_CATEGORY_ID; ?>) en

...
const VDM_CSV          = '/home/francobordo/public_html/import/feed/VDM/VDM_products_francobordo.csv';
const VDM_SFTP_HOST    = 'stftpamgprd01.blob.core.windows.net'; // SFTP Alliance Marine (Azure Blob)
const VDM_SFTP_PORT    = 22;
const VDM_SFTP_USER    = 'stftpamgprd01.amghubfrancobordo';
const VDM_SFTP_PASS    = 'lkYgzugus0nD0bQi/smYJLzSMY2RyFBS';
const VDM_SFTP_REMOTE  = 'products/VDM_products_francobordo.csv';
define('IMG_ABS_DIR',     dirname(dirname(__FILE__)) . '/images/productos/');
const NEW_CATEGORY_ID   = 1966; // "VDM Nuevos" (parent 0, status 0, creada 2026-07-07)
...
function vdmTranslateLabel($label, $to) {
	$out = (string) $label;
...
const UNIVERSE_ES = [
	...
	'ACCASTILLAGE'        => 'Accesorios de Cubierta',
...
/** Parsea números del feed VDM (decimal con punto; tolera coma europea). Null si no numérico. */
function vdmParseNum($v) {
	$v = trim((string) $v);
	if ($v === '') return null;
...
function vdmSlugify($text, $maxLen = 50) {
	$t = trim((string) $text);
...
function vdmBrandPrefix($rawBrand, $rawName = '') {
...
function resolveManufacturer($mysqli, $rawName, &$cache, &$createdLog, $dryRun) {
	...
	$display = vdmNormalizeManufacturer($rawName);
...
/** Subcategoría de staging por Universo bajo VDM Nuevos (status=0, oculta), con nombre ES+EN.
...
/** Mapa de duplicados — FILTRADO por origin VDM (multi-marca, como FNI/Lankhorst).
...
function vdmExtractMeasure($title) {
	$title = (string) $title;
...
/** Descarga VDM_products_francobordo.csv vía SFTP (Azure Blob). Devuelve ['ok'=>bool,...]. */
function downloadFeedFromSftp($localPath) {
	try {
		$dir = dirname($localPath);
		if (!is_dir($dir)) @mkdir($dir, 0775, true);
		$sftp = new SFTP(VDM_SFTP_HOST, VDM_SFTP_PORT, 30);
		if (!$sftp->login(VDM_SFTP_USER, VDM_SFTP_PASS)) return ['ok' => false, 'err' => 'login fallido'];
		$tmp = $localPath . '.tmp.' . uniqid();
		if (!$sftp->get(VDM_SFTP_REMOTE, $tmp)) return ['ok' => false, 'err' => 'get fallido'];
...
/** Lee el CSV VDM (pipe |, UTF-8, CON cabecera) mapeando por nombre de columna.
...
function loadCsvRows($path, &$statusSkipped = 0, &$badRows = 0) {
	...
	while (($r = fgetcsv($f, 0, '|', chr(34), '')) !== false) {
		...
		$rows[] = [
			...
			'EAN'        => vdmNormalizeEan($col($r, 'Barcode')),
...
function vdmDownloadImages($mysqli, $pid, array $row, $nameForSlug) {
	$urls = [];
...
<?php if ($isAction): ?>
	<h2>Importador VDM (Alliance Marine) — <?php echo $dryRun ? 'DRY-RUN (sin cambios)' : 'EJECUCIÓN REAL'; ?></h2>
	<p>
		<a href="<?php echo tep_href_link('import-vdm-altas.php'); ?>" class="xbutton small hv9">← Volver</a>
	</p>
...
if (NEW_CATEGORY_ID <= 0) { logMsg("ERROR: NEW_CATEGORY_ID sin fijar (crear categoría 'VDM Nuevos' y poner su id)"); goto end_action; }

if (!$skipDownload) {
	$mtime = file_exists(VDM_CSV) ? filemtime(VDM_CSV) : 0;
	logMsg("Descargando " . VDM_SFTP_REMOTE . " desde SFTP " . VDM_SFTP_HOST . " …" . ($mtime ? " (local actual: " . date('Y-m-d H:i', $mtime) . ", " . round(filesize(VDM_CSV)/1048576, 1) . " MB)" : ""));
	$dl = downloadFeedFromSftp(VDM_CSV);
	if ($dl['ok']) {
		logMsg("  ✓ descargado: " . round($dl['size']/1048576, 1) . " MB (mtime " . date('Y-m-d H:i', filemtime(VDM_CSV)) . ")");
	} else {
		logMsg("  ✗ descarga fallida: " . $dl['err'] . " — uso copia local existente si la hay");
		if (!file_exists(VDM_CSV)) { logMsg("ERROR: no hay copia local y SFTP falló"); goto end_action; }
	}
}

if (!file_exists(VDM_CSV)) { logMsg("ERROR: CSV no encontrado: " . VDM_CSV); goto end_action; }
logMsg("Leyendo CSV…");
$statusSkipped = 0; $badRows = 0;
$rows = loadCsvRows(VDM_CSV, $statusSkipped, $badRows);
...
logMsg("  → " . count($existing) . " referencias ya en BD (scope origin=vdm + EAN global + lista negra)");
...
foreach ($rows as $row) {
	...
	// Dedup: SKU VDM, referencia de fabricante y EAN
	...
	$net = vdmParseNum($row['NET_PRICE']);
	if ($net === null || $net <= 0) { $skippedBadPrice++; continue; }
	$pub = vdmParseNum($row['PUBLIC']);
	$cost = $net;
...
function vdmBuildTexts(array $row, $brandPrefix, $skipTranslation, $dryRun, &$translateFail) {
	$rawNameFr = $row['NAME_FR'] !== '' ? $row['NAME_FR'] : $row['NAME_EN'];
	$rawNameEn = $row['NAME_EN'] !== '' ? $row['NAME_EN'] : $row['NAME_FR'];
	$titleFr = vdmApplyBrandPrefix($brandPrefix, $rawNameFr);
	$titleEn = vdmApplyBrandPrefix($brandPrefix, $rawNameEn);
	...
	if (!$skipTranslation && !$dryRun) {
		...
		if ($tn !== '') $nameEs = vdmApplyBrandPrefix($brandPrefix, $tn); else $translateFail++;
...
foreach ($families as $parent => $items) {
	...
	try {
		// model = referencia del fabricante si la hay (mejor dedup/visible); reference_prov = SKU VDM (código de pedido)
		...
		list($imgOk, $subs) = vdmDownloadImages($mysqli, $pid, $cheap, $nameEs ?: $titleEn);
...
foreach ($standalone as $sku => $row) {
	...
	$brandPrefix = vdmBrandPrefix($row['BRAND'], $row['NAME_FR'] !== '' ? $row['NAME_FR'] : $row['NAME_EN']);
	$catId = getOrCreateSubcategory($mysqli, $row['_UNIV_ES'], $row['_UNIV_EN'], NEW_CATEGORY_ID, $dryRun, $subcatCache, $subcatCreated);

	list($nameEs, $descEs, $titleEn, $descEn) = vdmBuildTexts($row, $brandPrefix, $skipTranslation, $dryRun, $translateFail);
	...
	try {
		...
		list($imgOk, $subs) = vdmDownloadImages($mysqli, $pid, $row, $nameEs ?: $titleEn);
		if ($imgOk) { $nWithImg++; $nSubImgs += $subs; }
	...
	<form method="get" style="background:#f5f5f5;padding:15px;border-radius:5px;">
		<p>
			...
			<select name="universe" style="min-width:260px;">
				...
				<?php
				$univList = file_exists(VDM_CSV) ? listUniversesFromCsv(VDM_CSV) : [];
			...
			<select name="brand" style="min-width:300px;">
				...
				<?php
				$brandList = file_exists(VDM_CSV) ? listBrandsFromCsv(VDM_CSV) : [];
			...
			<?php if (!file_exists(VDM_CSV)): ?>
				<em style="color:#a00;">— CSV no descargado todavía; ejecuta una vez para popular los desplegables.</em>
			<?php endif; ?>
		</p>
		<p>
			<strong>SKUs específicos</strong> (opcional; acepta SKU VDM o referencia de fabricante; ignora filtros y máximo; si es variante importa toda la familia):<br>

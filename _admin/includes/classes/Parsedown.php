...
class Parsedown
{
    ...
    {
        [$name, $pattern] = $Line['text'][0] <= '-' ? ['ul', '[*+-]'] : ['ol', '\d+[.]'];
    ...
    {
        if ($Block['indent'] === $Line['indent'] && preg_match('/^'.$Block['pattern'].'(?:[ ]+(.*)|$)/', (string) $Line['text'], $matches))
        {
            if (isset($Block['interrupted']))
            {
                $Block['li']['text'] []= '';
    ...
    {
        if (str_contains((string) $Excerpt['text'], '>') && preg_match('/^<((mailto:)?\S+?@\S+?)>/i', (string) $Excerpt['text'], $matches))
        {
            $url = $matches[1];
    ...
    {
        if ($this->markupEscaped || !str_contains((string) $Excerpt['text'], '>'))
        {
            return null;
        }

        if ($Excerpt['text'][1] === '/' && preg_match('/^<\/\w*[ ]*>/s', (string) $Excerpt['text'], $matches))
        {
            return [
                'markup' => $matches[0],
                'extent' => strlen($matches[0]),
        ...
        if ($Excerpt['text'][1] !== ' ' && preg_match('/^<\w*(?:[ ]*'.$this->regexHtmlAttribute.')*[ ]*\/?>/s', (string) $Excerpt['text'], $matches))
        {
            return [
                'markup' => $matches[0],
                'extent' => strlen($matches[0]),

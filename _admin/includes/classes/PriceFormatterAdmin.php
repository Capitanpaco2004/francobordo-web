...
class PriceFormatter {
    ...
    public $specialPrice;
    public static $thePrice = -1;
	public static $taxClass = -1;
	public static $qtyBlocks = 1;
	public static $products_min_order_qty = 1;
	public static $price_breaks = [];
	public static $hasQuantityPrice = false;
	public static $hiPrice = -1;
	public static $lowPrice = -1;
    ...
    if (true == $this->hasQuantityPrice) {
        ...
        foreach($this->price_breaks as $key => $price_break) {
            ...
            if ($key == $no_of_price_breaks -1) {
                $lc_text .= '+&nbsp;</td>' . "\n";
            } elseif (($this->price_breaks[$key + 1]['products_qty'] - $this->price_breaks[$key]['products_qty']) > $qtyBlocks) {
  ...
  function getPriceDropDown() {
    ...
    $qtyBlocks = $this->getQtyBlocks();
      ...
      if ($this->price_breaks[0]['products_qty'] > $this->products_min_order_qty) {
        ...
        $dropdown_price_breaks[] = ['id' => $i, 'text' => $pb_text];
        $i++;
      } // end if ($this->price_breaks[0]['products_qty'] > $this->products_min_order_qty)
     ...
     for ($z = 0; $z < $no_of_price_breaks; $z++) {
       ...
       if ($z === $no_of_price_breaks - 1) {
           // last one
           $pb_text .= '+';
       } elseif (($this->price_breaks[$z + 1]['products_qty'] - $this->price_breaks[$z]['products_qty']) > $qtyBlocks) {
     ...
     return $dropdown . ('&nbsp;<span class="smalltext">' . PB_FROM . '</span>&nbsp;' . $currencies->display_price($this->lowPrice, tep_get_tax_rate($this->taxClass)) . "\n");
  }
}
?>

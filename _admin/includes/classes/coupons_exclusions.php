 ...
 */
  ...
  class coupons_exclusions {
  	...
  	public $type, $display_fields, $coupons_id, $table_name, $selected_options, $all_options;

    function __construct( $coupons_id, $type = 'products' ) {
    	...
    	if( tep_not_null( $coupons_id ) ) {
			...
			if( defined( 'TABLE_DISCOUNT_COUPONS_TO_'.strtoupper( (string) $type ) ) ) {
				$this->type = $type;
				$this->table_name = constant( 'TABLE_DISCOUNT_COUPONS_TO_'.strtoupper( (string) $type ) );
			} else {
				return;
			}
		...
		function save( $selected_options = [] ) {
      ...
      $result = tep_db_query( $sql );
  		...
  		if( tep_db_num_rows( $result ) > 0 ) {
				while( $row = tep_db_fetch_array( $result ) ) {
			      ...
			      $categories = [];
					...
					$selected_ids[] = $id;
			    $this->selected_options[] = [ 'text' => implode( $row, $separator ), 'id' => $id ];
				}
		...
		function get_all_options( $sql, $separator = ' :: '/*category exclusions*/, $category_separator = '->', $category_path = true/*end category exclusions*/, $selected_ids = [] ) {
			if (!tep_not_null( $sql )) {
                return false;
            }
      $result = tep_db_query( $sql );
	    if( tep_db_num_rows( $result ) > 0 ) {
				while( $row = tep_db_fetch_array( $result ) ) {
			      ...
			      $categories = [];
					...
					unset( $row['id'] ); //don't display in the list
			    $this->all_options[] = [ 'text' => implode( $row, $separator ), 'id' => $id ];
				}

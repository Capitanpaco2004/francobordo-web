...
if (!defined('FILENAME_PRODUCT_INFO')) define('FILENAME_PRODUCT_INFO', 'product_info.php');
  ...
  class recover_cart_sales {
    public $basedays;
    public $skipdays;
    public $infoBoxHeading;
    public $infoBoxContents;
    function __construct($basedays, $skipdays) {
      ...
      for ($i = 0; $i < $knt; $i++)
      {
        ...
        if ($curcus != $inrec['cid'])
        {
          ...
          if ($curcus != "" && !$skip) {
            ...
            $this->infoBoxContents[] = array(
              0 => array(
                ...
                'text' => '<a href="' . tep_href_link(FILENAME_RECOVER_CART_SALES, 'action=delete&customer_id=' . $curcus . '&tdate=' . $this->basedays . '&sdate=' . $this->skipdays) . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a>'));
          }
          ...
          if ($curcus != "")
            ...
            $ccqueryraw = 'select orders_id, orders_status from ' . TABLE_ORDERS . ' where (customers_id = ' . (int)$curcus . ' OR customers_email_address like "' . $inrec['email'] .'" or customers_name like "' . $inrec['fname'] . ' ' . $inrec['lname'] . '") and date_purchased >= "' . $beforeDate . '"';
            $ccquery = tep_db_query($ccqueryraw);
			      if (tep_db_num_rows($ccquery) > 0)
			      {
				      ...
				      {
                $ccquery2raw = 'select products_id from ' . TABLE_ORDERS_PRODUCTS . ' where orders_id = ' . (int)$orec['orders_id'] . ' AND products_id = ' . (int)$inrec['pid'];
                $ccquery2 = tep_db_query($ccquery2raw);
					      if(tep_db_num_rows($ccquery2) > 0)
					      {
						      ...
						      else
						      {
							      ...
							      $ccquery3 = tep_db_query("select orders_status_name from " . TABLE_ORDERS_STATUS . " where language_id = " . (int)$languages_id . " AND orders_status_id = " . (int)$orec['orders_status'] );
      			
							      if($srec = tep_db_fetch_array($ccquery3))
        ...
        if ($inrec['qty'] != 0)
        {
          ...
          $this->infoBoxContents[] = array(
            ...
            2 => array(
              ...
              'text' => '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'action=new_product_preview&read=only&pID=' . $inrec['pid'] . '&origin=' . FILENAME_RECOVER_CART_SALES . '?page=' . ($_GET['page'] ?? 1), 'NONSSL') . '"><b>' . $inrec2['name'] . '</b></a>' . $prodAttribs),
            3 => array(
      ...
      $this->infoBoxContents[] = array(
        ...
        unset($email);
          ...
          $inrec = tep_db_fetch_array($query1);
              ...
              $this->infoBoxContents[] = array(
                0 => array(
                  ...
                  'text' => '<a href="' . tep_href_link(FILENAME_RECOVER_CART_SALES, "action=delete&customer_id=" . $cid . "&tdate=" . $this->basedays . "&sdate=" . $this->skipdays) . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a>'));
			      }
            ...
            $this->infoBoxContents[] = array(
		      ...
		      $query2 = tep_db_query("select   p.products_price price, p.products_image,
												    p.products_tax_class_id taxclass,
												    p.products_model model,
                                        pd.products_name name
                                from    " . TABLE_PRODUCTS . " p,
            ...
            $this->infoBoxContents[] = array(
              ...
              1 => array(
                ...
                'text' => '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'action=new_product_preview&read=only&pID=' . $inrec['pid'] . '&origin=' . FILENAME_RECOVER_CART_SALES . '?page=' . ($_GET['page'] ?? 1), 'NONSSL') . '">' . $inrec2['name'] . '</a>'),
              2 => array(
          ...
          $email = '<span style="font-family: Verdana, Arial, sans-serif; font-size: 11px;">';
			...
			tep_mail('', $outEmailAddr, EMAIL_TEXT_SUBJECT, $email, STORE_OWNER, EMAIL_FEEDBACK);

          //Copy
		      //$outEmailAddr = EMAIL_FEEDBACK;
		      //tep_mail('', $outEmailAddr, EMAIL_TEXT_SUBJECT, $email, STORE_OWNER, EMAIL_FEEDBACK);

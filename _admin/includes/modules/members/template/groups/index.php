...
<div class="oeBox oeTable column a12 row ax">
	<div class="oeWrpr">
		<div class="oeTitu"><i class="fa fa-eye"></i> <?php echo ADMIN_MEMBERS_HEADING_SUBTITLE_GROUPS_LIST ?></div>
		<form method="post" action="<?php echo tep_href_link( $sUrlPage, 'action=groups' ) ?>" class="oeCntd row ax">
			...
			<table class="xform">
				<thead>
					<tr>
						...
						<th><?php echo ADMIN_MEMBERS_TABLE_HEADING_GROUP ?></th>
						<th width="125"><?php echo ADMIN_MEMBERS_TABLE_ACTIONS ?></th>
					</tr>
				</thead>
				<tbody>
				<?php while( $group = tep_db_fetch_array( $aRows ) ): ?>
					<tr data-dblclick="<?php echo tep_href_link( $sUrlPage, 'action=groups_crud&id=' . $group['admin_groups_id'] ) ?>">
						<td class="chck" align="center"><input type="checkbox" id="id_<?php echo $group['admin_groups_id'] ?>" name="id[]" value="<?php echo $group['admin_groups_id'] ?>"/><label for="id_<?php echo $group['admin_groups_id'] ?>"><span></span></label></td>
						<td><?php echo $group['admin_groups_name'] ?></td>
						<td>
							<div class="drop xfselect">
								<div><?php echo ADMIN_MEMBERS_TABLE_ACTIONS ?></div>
								<ul class="down down-dngt">
									<li><a href="<?php echo tep_href_link( $sUrlPage, 'action=groups_crud&id=' . $group['admin_groups_id'] ) ?>" class="hv"><i class="fa fa-pencil"></i><?php echo ADMIN_MEMBERS_TEXT_EDIT ?></a></li>
									<li><a data-confirm="<?php echo ADMIN_MEMBERS_TEXT_DELETES_CONFIRM ?>" href="<?php echo tep_href_link( $sUrlPage, 'action=groups_delete&id=' . $group['admin_groups_id'] ) ?>" class="hv"><i class="fa fa-trash"></i><?php echo ADMIN_MEMBERS_TEXT_DELETE ?></a></li>
								</ul>
							</div>
						</td>
					</tr>
...
<form action="<?php echo tep_href_link( $sUrlPage ); ?>" method="post" id="fltr-lstd" class="vntn-form mfp-hide zoom-anim-dialog oeBox mfp-white">
	...
	<div class="oeWrpr">
		<div class="oeTitu"><i class="far fa-user-crown"></i> <?php echo ADMIN_MEMBERS_HEADING_SUBTITLE_GROUPS_LIST ?></div>
		<div class="oeCntd row ax xform xform-horizontal">
			...
			<div class="column a12 tright">
				<?php echo ($sWhere != '' ? '<a href="' . tep_href_link( $sUrlPage, 'action=groups', tep_get_all_get_params( [ 'filter' ] ) ) . '" class="xbutton hv9 rojo small"><i class="fas fa-times"></i> Eliminar</a> ' : ''); ?>

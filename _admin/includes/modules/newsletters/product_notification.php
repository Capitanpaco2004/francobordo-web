...
//--></script>';
    ...
    function send($newsletter_id, $send_to_customer_groups) {
      ...
      if (isset($_POST['global']) && ($_POST['global'] == 'true')) {
        ...
        $products_query = tep_db_query("select distinct pn.customers_id, c.customers_firstname, c.customers_lastname, c.customers_email_address from " . TABLE_CUSTOMERS . " c, " . TABLE_PRODUCTS_NOTIFICATIONS . " pn where c.customers_id = pn.customers_id and customers_group_id in (" . $send_to_customer_groups . ")");
	    } else {
        ...
        while ($products = tep_db_fetch_array($products_query)) {
	      ...
	      // BOF Separate Pricing Per Customer
	    if (tep_not_null($send_to_customer_groups)) {
        $customers_query = tep_db_query("select c.customers_id, c.customers_firstname, c.customers_lastname, c.customers_email_address from " . TABLE_CUSTOMERS . " c, " . TABLE_CUSTOMERS_INFO . " ci where c.customers_id = ci.customers_info_id and ci.global_product_notifications = '1' and customers_group_id in (" . $send_to_customer_groups . ")");
	    } else {
        ...
        $ids = implode(',', $chosen);
	      // BOF Separate Pricing Per Customer
	    if (tep_not_null($send_to_customer_groups)) {
        $products_query = tep_db_query("select distinct pn.customers_id, c.customers_firstname, c.customers_lastname, c.customers_email_address from " . TABLE_CUSTOMERS . " c, " . TABLE_PRODUCTS_NOTIFICATIONS . " pn where c.customers_id = pn.customers_id and pn.products_id in (" . $ids . ") and customers_group_id in (" . $send_to_customer_groups . ")");
	    } else {

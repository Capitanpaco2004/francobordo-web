...
function getSqlSearchProducts_bd($sSearch)
{
	...
	$sJoins .= 'INNER JOIN ' . TABLE_PRODUCTS_DESCRIPTION . ' pd ON (p.products_id = pd.products_id AND pd.language_id = ' . $languages_id . ') ';
...
function getNameProductsById( $nIdProduct )
{
	global $languages_id;

	$aAux = tep_db_query( 'SELECT products_name FROM products_description WHERE language_id = ' . $languages_id . ' AND products_id = ' . (int)$nIdProduct );
	$aAux = tep_db_fetch_array( $aAux );

	return $aAux['products_name'];
}

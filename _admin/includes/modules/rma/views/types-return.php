<?php
if (intval($_GET['id']) > 0) {
    $aDato = rmaGetTypesReturn(intval($_GET['id']));
} else {
    $aDatos = rmaGetTypesReturn();
}
?>
...
<div class="row ax">
    <div class="oeBox column a03 T06 row ax">
        <div class="oeWrpr">
            ...
            <form class="oeCntd rows sp10 ax xform" method="post" action="<?php echo tep_href_link('rma.php', 'action=save-types-return'); ?>">
                <label class="column a03 ">Agencia:</label><div class="column a09">
                    <select name="agencia" class="skip">
                        <option value="1"<?php echo (intval($aDato['agencia']) == 1 ? ' selected': ''); ?>>Sí</option>
                        <option value="0"<?php echo (intval($aDato['agencia']) == 0 ? ' selected': ''); ?>>No</option>
                    </select>
                </div>

                <label class="column a03 ">Coste:</label><div class="column a09"><input autocomplete="off" name="coste" value="<?php echo $aDato['price_cost']; ?>" type="text"></div>
                <hr class="column a12" />
                        ...
                        <?php foreach ($aDatos as $key => $aDato): ?>
                            <tr class="oeTrProduct">
                                <td style="width: 20px; text-align: center;">
                                    ...
                                    <a href="<?php echo tep_href_link('rma.php', 'action=remove-types-return&id=' . $aDato['id']); ?>" class="tnegro rmaRemove"><i class="fa fa-trash tnegro oeDelete" style="cursor: pointer;"></i></a>
                                </td>
                                <td>
                                    <?php echo $aDato['id']; ?>
                                </td>
                                ...
                                <td>
                                    <?php echo(intval($aDato['agencia']) == 1 ? 'Sí' : 'No'); ?>
                                </td>
                                <td>
                                    <?php echo $aDato['price_cost']; ?>€
                                </td>
                                <td>
                                    <a href="<?php echo tep_href_link('rma.php', 'action=active-types-return&id=' . $aDato['id'].'&active='.(intval($aDato['active']) == 0 ? 1 : 0)); ?>" class="<?php echo intval($aDato['active']) == 0 ? 'rmaInactive' : 'rmaActive'; ?> rmaActiveLink">

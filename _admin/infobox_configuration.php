<?php
/* 
  $Id: configuration.php,v 1.1.1.1 2002/11/28 20:56:51 wilt Exp $
  ...
  Patches:
  - tom 001: use set_function to edit 'left' / 'right'
  ...
  if ($_GET['action']) {
    switch ($_GET['action']) {
      ...
      case 'save':
        ...
        $configuration_value  = tep_db_prepare_input($_POST['configuration'][active]);
        $configuration_column = tep_db_prepare_input($_POST['configuration'][column]);
        // tom 001: End
...
<?php
  ...
  // tom 001:
  // $configuration_query = tep_db_query("select configuration_id, configuration_title, configuration_value, configuration_column, location from " . theme_configuration . " where configuration_group_id = '" . $_GET['gID'] . "' order by configuration_column, location ");// location ");configuration_column ");
  $configuration_query = tep_db_query("select configuration_id, configuration_title, configuration_value, configuration_column, location, set_function from " . theme_configuration . " where configuration_group_id = '" . $_GET['gID'] . "' order by configuration_column, location ");// location ");configuration_column ");
...
<?php
    if ($configuration['configuration_value'] == 'yes') {
      echo tep_image(DIR_WS_IMAGES . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_INFOBOX_CONFIGURATION, 'action=setflag&flag=no&cID=' . $configuration['configuration_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
    } else {
...
<?php
  ...
  switch ($_GET['action']) {
    case 'edit':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . '&nbsp;<a href="' . tep_href_link(FILENAME_INFOBOX_CONFIGURATION, 'gID=' . $_GET['gID'] . '&cID=' . $cInfo->configuration_id) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
...
case 'new':
      ...
      $contents[] = array('text' => '<br><a href="javascript:popupWindow(\'' . tep_href_link(FILENAME_POPUP_INFOBOX_HELP,'action=filename') . '\')">' . tep_image(DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a><b> ' . TEXT_INFOBOX_NEW_FILENAME . '</b><br> ' . tep_draw_input_field('configuration_title','what\'s new','','true'));
      $contents[] = array('text' => '<br><a href="javascript:popupWindow(\'' . tep_href_link(FILENAME_POPUP_INFOBOX_HELP,'action=heading') . '\')">' . tep_image(DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a><b> ' . TEXT_INFOBOX_NEW_NAME . '</b><br>' . tep_draw_input_field('box_heading','What\'s New','','true'));
      $contents[] = array('text' => '<br><a href="javascript:popupWindow(\'' . tep_href_link(FILENAME_POPUP_INFOBOX_HELP,'action=define') . '\')">' . tep_image(DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a><b> ' . TEXT_INFOBOX_NEW_KEY . '</b><br>' . tep_draw_input_field('configuration_key','BOX_HEADING_????','','true'));
      $contents[] = array('text' => '<br><a href="javascript:popupWindow(\'' . tep_href_link(FILENAME_POPUP_INFOBOX_HELP,'action=column') . '\')">' . tep_image(DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a><b> ' . TEXT_INFOBOX_NEW_COLUMN . '</b><br>' . tep_draw_input_field('configuration_column','','maxlength="5"','true'));
      $contents[] = array('text' => '<br><a href="javascript:popupWindow(\'' . tep_href_link(FILENAME_POPUP_INFOBOX_HELP,'action=position') . '\')">' . tep_image(DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a><b> ' . TEXT_INFOBOX_ORDER . '</b><br>' . tep_draw_input_field('location'));
      $contents[] = array('text' => '<br><a href="javascript:popupWindow(\'' . tep_href_link(FILENAME_POPUP_INFOBOX_HELP,'action=active') . '\')">' . tep_image(DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a><b> ' . TEXT_INFOBOX_ACTIVATE . '</b>' . $value_field_value . '</b>');
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_insert.png', IMAGE_INSERT) . '&nbsp;<a href="' . tep_href_link(FILENAME_INFOBOX_CONFIGURATION, 'gID=' . $_GET['gID']) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . '&nbsp;<a href="' . tep_href_link(FILENAME_INFOBOX_CONFIGURATION, 'gID=' . $_GET['gID'] . '&cID=' . $cInfo->configuration_id) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;

    default:
      if (is_object($cInfo)) {
        $heading[]  = array('text' => '<b>' . $cInfo->configuration_title . '</b>');
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_INFOBOX_CONFIGURATION, 'gID=' . $_GET['gID'] . '&cID=' . $cInfo->configuration_id . '&action=edit') . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_INFOBOX_CONFIGURATION, 'gID=' . $_GET['gID'] . '&cID=' . $cInfo->configuration_id . '&action=delete') . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a><br><br><a href="' . tep_href_link(FILENAME_INFOBOX_CONFIGURATION, 'gID=' . $_GET['gID'] . '&action=new') . '">' . tep_image_button('button_new_infobox.png', IMAGE_NEW_INFOBOX) . '</a> ');

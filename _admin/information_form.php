<?PHP
  /*
  Module: Information Pages Unlimited
          File date: 2007/02/17
          Based on the FAQ script of adgrafics
          Adjusted by Joeri Stegeman (joeri210 at yahoo.com), The Netherlands

  osCommerce, Open Source E-Commerce Solutions
  http://www.oscommerce.com

  Released under the GNU General Public License
  */
?>
<tr>
	<td class="pageHeading"><?php echo $title ?></td>
</tr>
<tr>
	<td>

<table border="0" cellpadding="0" cellspacing="2" width="100%">
<?php
if(!strstr($info_group['locked'], 'visible')) {
?>

	<tr>
		<td class="main"><?php echo ENTRY_STATUS; ?></td>
		<td class="main"><?php echo tep_draw_radio_field('visible', '1', true, $edit['visible']) . '&nbsp;&nbsp;' . STATUS_ACTIVE . '&nbsp;&nbsp;' . tep_draw_radio_field('visible', '0', false, $edit['visible']) . '&nbsp;&nbsp;' . STATUS_INACTIVE; ?></td>
	</tr>
	<tr>
		<td colspan="2" height="10"></td>
	</tr>
<?php
}

if(!strstr($info_group['locked'], 'parent_id')) {
?>
	<tr>
		<td class="main"><?php echo ENTRY_PARENT_PAGE; ?></td>
		<td class="main">
<?php
  if ((sizeof($data) > 0) ) {
	$options = '<option value="0">-</option>';
	foreach($data as $key => $val) {
		$selected = ($val['information_id'] == $edit['parent_id']) ? 'selected' : '';
		$options .= '<option value="' . $val['information_id'] . '" ' . $selected . '>' . $val['information_title'] . '</option>';
	}
	echo '<select name="parent_id">' . $options . '</select>';
  } else {
  	echo '<span class="messageStackError">' . WARNING_PARENT_PAGE .'</span>';
  }
?>
		</td>
	</tr>
	<tr>
		<td colspan="2" height="10"></td>
	</tr>
<?php
}

if(!strstr($info_group['locked'], 'sort_order')) {
?>
	<tr>
		<td class="main"><?php echo ENTRY_SORT_ORDER;?></td>
		<td><?php if ($edit['sort_order']) {$no=$edit['sort_order'];}; echo tep_draw_input_field('sort_order', "$no", 'size=3 maxlength=4'); ?></td>
	</tr>
	<tr>
		<td colspan="2" height="10"></td>
	</tr>
<?php
}

if(!strstr($info_group['locked'], 'information_title')) {
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
?>
	<tr>
		<td class="main"><?php if ($i == 0) echo ENTRY_TITLE;?><br></td>
		<td><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('information_title[' . $languages[$i]['id'] . ']', (($languages[$i]['id'] == $languages_id) ? stripslashes($edit['information_title']) : tep_get_information_entry($information_id, $languages[$i]['id'], 'information_title')), 'maxlength=255'); ?></td>
	</tr>
	<tr>
		<td colspan="2" height="10"></td>
	</tr>

<?php
    }
}

if(!strstr($info_group['locked'], 'information_description')) {
	for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
?>
	<tr>
		<td valign="top" class="main" width="100" colspan="2"><?php if ($i == 0) echo ENTRY_DESCRIPTION; ?></td>
		<table border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td class="main" valign="top"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']); ?>&nbsp;</td>
			<td class="main" width="100%"><?php
				echo tep_draw_textarea_field_tinymce('information_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($languages[$i]['id'] == $languages_id) ? stripslashes($edit['information_description']) : tep_get_information_entry($information_id, $languages[$i]['id'], 'information_description')));
		 ?>
		 </td>
		</tr>
		</table>
	</tr>
<?php
    }
}
?>
	<?php
		echo '<tr>
			<td valign="top" class="main" width="100" colspan="2"><b><br><br><b>Opciones SEO:</b></b>';

			// Campos del formulario
			$aCampos = array(
				array( 'title' => 'Título', 'text' => 'Título que se muestra en la cabecera del navegador.', 'row' => 'information_seo_title' ),
				array( 'title' => 'Palabras claves', 'text' => 'Palabras, frases clave o términos de búsqueda que los buscadores usaran para encontrar información.', 'row' => 'information_seo_keywords' ),
				array( 'title' => 'Descripción', 'text' => 'Descripcion que nos ayudará a indicar cúal es el contenido de nuestra página. Recomendamos que tenga entre 70 y 156 caracteres (incluyendo espacios).', 'row' => 'information_seo_description' )
			);

			// Obtenemos los valores de los campos seo
			if( $_GET['information_id'] )
			{
				$aDatos = tep_db_query( 'select language_id, information_seo_title, information_seo_keywords, information_seo_description
										 from information
										 where information_id = ' . $_GET['information_id'] );

				while( $aDato = tep_db_fetch_array( $aDatos ) )
					$aDatosSeo[$aDato['language_id']] = $aDato;
			}

			// Recorremos los idiomas
			foreach( $languages as $aIdioma )
			{
				echo '<div style="display: block;" class="tab-seo-idma" id="seo-' . $aIdioma['id'] . '">';

				// Recorremos los campos
				foreach( $aCampos as $aCampo )
				{
					echo '<div class="formRow" style="position: relative; padding: 5px 10px 5px 40px; border: none; width: 700px;">';
						echo tep_image( DIR_WS_CATALOG_LANGUAGES . $aIdioma['directory'] . '/images/' . $aIdioma['image'], $aIdioma['name'], '', '', 'style="position: absolute; top: 15px; left: 0px;"' );

						echo '<div class="grid3" style="margin: 0px;">';
							echo '<label>' . $aCampo['title'] . '</label>';
							echo '<span class="note" style="display: block; width: 100%; float: left; margin-top: -8px; font-style: italic; white-space: normal;">' . $aCampo['text'] . '</span>';
						echo '</div>';
						echo '<div class="grid9">';

							switch( $aCampo['row'] )
							{
								case 'information_seo_description':
								case 'information_seo_keywords':
									echo tep_draw_textarea_field( $aCampo['row'] . '[' . $aIdioma['id'] . ']', '', 5, 5, $aDatosSeo[$aIdioma['id']][$aCampo['row']], 'style="margin: 0px 0px 10px;"' );
								break;

								default:
									echo tep_draw_input_field( $aCampo['row'] . '[' . $aIdioma['id'] . ']', $aDatosSeo[$aIdioma['id']][$aCampo['row']], 'style="width: 100%; margin: 0px 0px 10px;"' ) . '<br/>';
								break;
							}

						echo '</div>';
						echo '<div class="clear"></div>';
					echo '</div>';
				}

				echo '</div>';
			}

		echo '</td></tr>';
	?>

	<tr>
		<td colspan="2" align="right"><?php
				// Decide when to show the buttons (Determine or 'locked' is active)
				if( (empty($info_group['locked'])) || ($_GET['information_action'] == 'Edit')) {
					echo tep_image_submit('button_insert.png', IMAGE_INSERT);
				}
				echo '&nbsp;<a href="' . tep_href_link(FILENAME_INFORMATION_MANAGER, "gID=$gID", 'NONSSL') . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>';
			?></td>
	</tr>
</table>


	</td>
</tr>
</form>

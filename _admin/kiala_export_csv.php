...
// create the CSV file
$csvName = 'orders-' . date("Ymd") . '-' . date("His") . '.csv';
...
foreach ($tabOrders as $order_id) {
	...
	$customer_fisrtname = $customer['customers_firstname'];
	...
	$customerLocality = $country_array[$order->delivery['country']];
	...
	$data = array(
		...
		'customerFirstName' => mb_convert_encoding($customer_fisrtname ?? '', 'UTF-8', 'ISO-8859-1'),
		'customerTitle' => '',
...
header("Content-disposition: filename=". $csvName .".csv");
echo $csvstring;
*/

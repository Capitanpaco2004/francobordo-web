...
// page nn button
      for ($jump_to_page = 1 + (($cur_window_num - 1) * $max_page_links); ($jump_to_page <= ($cur_window_num * $max_page_links)) && ($jump_to_page <= $this->number_of_pages); $jump_to_page++) {
        if ($jump_to_page == $this->current_page_number) {
          ...
          $display_links_string .= '&nbsp;<a href="' . tep_href_link(basename($PHP_SELF), $parameters . $this->page_name . '=' . $jump_to_page, $request_type) . '" class="pageResults" title="' . $jump_to_page . ' "><u>' . $jump_to_page . '</u></a>&nbsp;';
        }
      }

// next window of pages
      if ($cur_window_num < $max_window_num) $display_links_string .= '<a href="' . tep_href_link(basename($PHP_SELF), $parameters . $this->page_name . '=' . (($cur_window_num) * $max_page_links + 1), $request_type) . '" class="pageResults" title=" ' . $max_page_links . ' ">...</a>&nbsp;';

// next button
      if (($this->current_page_number < $this->number_of_pages) && ($this->number_of_pages != 1)) $display_links_string .= '&nbsp;<a href="' . tep_href_link(basename($PHP_SELF), $parameters . 'page=' . ($this->current_page_number + 1), $request_type) . '" class="pageResults" title="Pagina Siguiente"><u>' . PREVNEXT_BUTTON_NEXT . '</u></a>&nbsp;';
...
// Return table heading with sorting capabilities
  ...
  if ((isset($_GET['action'])) && ($_GET['action'] == 'export')) {
	if (isset($_GET['manufacturers_id']) && ($_GET['manufacturers_id'] != '') && (!isset($_GET['filter_id']))) {
	  ...
	  $xls_header = 'Margen de Beneficio: ' . $manufacturer['name'];
	  ...
	  $xls_header = 'Margen de Beneficio: ' . $manufacturer['name'] . ' - ' . $category['name'];
	} else {
	  $xls_header = 'Margen de Beneficio: Todos los productos';
	  }

	  
	$sql = stripslashes($_SESSION['sql_recorder_margin']);
	$csv_query = tep_db_query($sql);
...
?>
<table border="0" width="100%" cellspacing="0" cellpadding="2" class="table-page">
  <tr>
    <td>&nbsp;</td>
  </tr>
  <tr>
...
$_SESSION['sql_recorder_margin'] = $listing_sql;	
?>
		      <td align="right"><table><tr><td><?php echo tep_draw_form('export_to_file', FILENAME_MARGIN_REPORT, 'get', '') . tep_draw_hidden_field('action', 'export') . $get_vars;?><input type="text" name="file" value="Nombre_del_Fichero" onfocus="javascript:if(this.value=='Nombre_del_Fichero') {this.value='';}" onblur="javascript:if(this.value=='') {this.value='Nombre_del_Fichero'}"></td><td><?php echo '<input type="image" name="submit" src="' . DIR_WS_LANGUAGES . $language . '/images/buttons/button_export.gif" alt="' . TEXT_EXPORT_BUTTON . '"></form>'; ?></td></tr></table></td>
		  </tr>
        </table>
		</td>
      </tr>

<?php
/*
  $Id: products_specifications.php v1.1 20101028 kymation
...
// Define variables for Selected items and Paths
  ...
  $svalue = (isset ($_GET['svalue']) ? (int) $_GET['svalue'] : 0); // Show Values
      ...
      case 'copy_group_confirm':
        ...
        if (isset ($_POST['specification_group_id']) && $_POST['specification_group_id'] != 0) {
          ...
          if (isset ($_POST['copy_specs']) && $_POST['copy_specs'] == 'True') {
          	// print 'Copy specs: ' . $_POST['copy_specs'] . '<br>';
          	// print 'Group ID: ' . $new_group_id . '<br>';
            ...
            while ($specs_array = tep_db_fetch_array ($specs_query) ) {
              $sql_data_array = array ('specification_group_id' => $new_group_id,
                                       ...
                                       'show_feeder' => $specs_array['show_feeder'],
              ...
              $specs_description_query_raw = "
                ...
                from
                  " . TABLE_SPECIFICATION_DESCRIPTION . "
                where
                  specifications_id = '" . $new_specs_id . "'
              ";
              // print $specs_description_query_raw . "<br>\n";
              $specs_description_query = tep_db_query ($specs_description_query_raw);

              while ($specs_description = tep_db_fetch_array ($specs_description_query) ) {
                $sql_data_array = array ('specifications_id' => $new_specs_id,
                                         'language_id' => $specs_description['language_id'],
                                         'specification_name' => $specs_description['specification_name'],
                                         'specification_description' => $specs_description['specification_description'],
                                         'specification_prefix' => $specs_description['specification_prefix'],
                                         'specification_suffix' => $specs_description['specification_suffix']
                                        );
                tep_db_perform (TABLE_SPECIFICATION_DESCRIPTION, $sql_data_array);
              } // while ($specs_description
            } // while ($specs_array
          ...
          if (isset ($_POST['copy_filter']) && $_POST['copy_filter'] == 'True' && isset( $specifications_ids )) {
            foreach ($specifications_ids as $specs_id) {
          ...
          if (isset ($_POST['copy_values']) && $_POST['copy_values'] == 'True' && isset( $specifications_ids )) {
            foreach ($specifications_ids as $specs_id) {
...
// Specifications section
      ...
      // Use this Specification in the Feeders
      case 'flag_feeder_spec':
        if ( ($flag == '0' || $flag == '1') && $specification_id != 0) {
          $db_flag = ($flag == '1') ? 'True' : 'False';
          tep_db_query ("update " . TABLE_SPECIFICATION . "
                         set show_feeder = '" . $db_flag . "'
                         where specifications_id = '" . $specification_id . "'
                      ");
        }
      ...
      case 'copy_specification_confirm':
        ...
        if (isset ($_POST['specifications_id']) && $_POST['specifications_id'] != 0) {
          ...
          $sql_data_array = array ('specification_group_id' => $specification_array['specification_group_id'],
                                   ...
                                   'show_feeder' => $specification_array['show_feeder'],
          ...
          $specification_description_query_raw = "
            select
              language_id,
              specification_name,
              specification_description,
            ...
            where
              specifications_id = '" . $specification_id . "'
          ";
          $specification_description_query = tep_db_query( $specification_description_query_raw );

          while( $specification_description = tep_db_fetch_array( $specification_description_query ) ) {
            $sql_data_array = array ('specifications_id' => $specification_id_copy,
                                     'language_id' => $specification_description['language_id'],
                                     'specification_name' => $specification_description['specification_name'],
                                     'specification_description' => $specification_description['specification_description'],
                                     'specification_prefix' => $specification_description['specification_prefix'],
                                     'specification_suffix' => $specification_description['specification_suffix']
                                    );
            tep_db_perform (TABLE_SPECIFICATION_DESCRIPTION, $sql_data_array);
          } // while( $specification_array
        } // if ($specification_id
      ...
      case 'delete_specification_confirm':
        if ( (int) $_POST['specifications_id'] != 0) {
          ...
          // Check if we have any filters attached to the specification
          $filters_query_raw = "select specification_filters_id
                                from " . TABLE_SPECIFICATIONS_FILTERS . "
...
// Filters section
      ...
      // Update the Manufacturers list in Filters
...
// Specification Values section
      ...
      case 'edit_value_confirm':
        ...
        tep_redirect (tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1&vid=' . $specification_values_id) );
        break;
      ...
      case 'move_value_confirm':
        ...
        tep_redirect (tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $move_to_value_id . '&svalue=1&vid=' . $specification_values_id) );
        break;
      ...
      case 'copy_value_confirm':
        ...
        tep_redirect (tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $copy_to_value_id . '&svalue=1&vid=' . $new_specification_values_id) );
        break;
      ...
      case 'delete_value_confirm':
        ...
        tep_redirect (tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1') );
        break;
...
//   These set up data used in the right column box
      ...
      case 'edit_specification':
        ...
        $static_array = array (array ('id' => '',
                               ...
                               array ('id' => 'combi',
                                      'text' => TEXT_COMBI),
                               array ('id' => 'products_options',
                                      'text' => TEXT_OPTIONS),
                               array ('id' => 'popup_image',
                                      'text' => TEXT_POPUP),
                               array ('id' => 'buy_now',
                                      'text' => TEXT_BUY_NOW),
        ...
        while ($row = tep_db_fetch_array ($describe_query) ) {
          switch ($row['Field']) {
            ...
            default:  // Show everything else by formatting the field name
              $title_string = preg_replace ('/_/i', ' ', $row['Field']);
              $title_string = ucwords ($title_string);
              $products_array[] = array ('id' => $row['Field'],
                                         'text' => $title_string
              );
      ...
      case 'copy_value':
        $specifications_query_raw = "
          ...
          where s.enter_values != 'text'
            and sd.language_id = '" . $languages_id . "'
        ";
        // print $specifications_query_raw . "<br>\n";
        $specifications_query = tep_db_query ($specifications_query_raw);

        $specifications_list = array();
        while ($specifications_data = tep_db_fetch_array ($specifications_query) ) {
          $specifications_list[] = array ('id' => $specifications_data['specifications_id'],
                                         'text' => $specifications_data['specification_name']
                                         );
        }
        break;
...
// Select the listings in the center section of the page
...
?>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $categories['specification_group_id']) . '">' . tep_image (DIR_WS_ICONS . 'folder.png', ICON_FOLDER) . '</a>&nbsp;<b>' . $categories['specification_group_name'] . '</b>'; ?></td>
                <td class="dataTableContent" align="center">
<?php
      if ($categories['show_comparison'] == 'True') {
        echo tep_image (DIR_WS_IMAGES . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_comparison&flag=0&sgid=' . $categories['specification_group_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_comparison&flag=1&sgid=' . $categories['specification_group_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image (DIR_WS_IMAGES . 'icon_status_red.png', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?>
                </td>
                <td class="dataTableContent" align="center">
<?php
      if ($categories['show_products'] == 'True') {
        echo tep_image (DIR_WS_IMAGES . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_products&flag=0&sgid=' . $categories['specification_group_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_products&flag=1&sgid=' . $categories['specification_group_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image (DIR_WS_IMAGES . 'icon_status_red.png', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?>
                </td>
                <td class="dataTableContent" align="center">
<?php
      if ($categories['show_filter'] == 'True') {
        echo tep_image (DIR_WS_IMAGES . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_filters&flag=0&sgid=' . $categories['specification_group_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_filters&flag=1&sgid=' . $categories['specification_group_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image (DIR_WS_IMAGES . 'icon_status_red.png', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?>
                </td>
...
<?php
        if (isset ($cInfo) && is_object ($cInfo) && ($categories['specification_group_id'] == $cInfo->specification_group_id) ) {
          ...
          echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path. '&sgid=' . $categories['specification_group_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a>';
        }
?>
                &nbsp;</td>
              </tr>
              ...
              <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td align="right" class="smallText"><?php echo tep_draw_button(IMAGE_NEW_SPECIFICATION_GROUP, 'triangle-1-e', tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=new_group'), 'primary'); ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
<?php
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="center"><?php echo TABLE_HEADING_IN_FEEDER; ?></td>
...
<?php
    ...
    $specifications_query_raw = "
      ...
      from
        ...
        join " . TABLE_SPECIFICATION_DESCRIPTION . " sd
          on sd.specifications_id = sp.specifications_id
       where
         sd.language_id = '" . $languages_id . "'
    ...
    while ($specifications = tep_db_fetch_array ($specifications_query) ) {
      ...
      if ($specifications['show_filter'] == 'True' && $specifications['filter_class'] != 'none') {
        $filter_link = '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specifications['specifications_id']) . '">' . tep_image (DIR_WS_ICONS . 'folder.png', ICON_FOLDER) . '</a>';
      }

      $value_link = tep_image (DIR_WS_ICONS . 'blank.png', ICON_BLANK);
      if ($specifications['enter_values'] != 'text') {
        $value_link = '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specifications['specifications_id'] . '&svalue=1') . '">' . tep_image (DIR_WS_ICONS . 'folder_blue.gif', ICON_FOLDER) . '</a>';
      }
?>
...
<?php
      if ($specifications['show_comparison'] == 'True') {
        echo tep_image (DIR_WS_IMAGES . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_comparison_spec&flag=0&sgpath=' . $specs_group_path . '&spid=' . $specifications['specifications_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_comparison_spec&flag=1&sgpath=' . $specs_group_path . '&spid=' . $specifications['specifications_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image (DIR_WS_IMAGES . 'icon_status_red.png', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?>
                </td>
                <td class="dataTableContent" align="center">
<?php
      if ($specifications['show_products'] == 'True') {
        echo tep_image (DIR_WS_IMAGES . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_products_spec&flag=0&sgpath=' . $specs_group_path . '&spid=' . $specifications['specifications_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_products_spec&flag=1&sgpath=' . $specs_group_path . '&spid=' . $specifications['specifications_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image (DIR_WS_IMAGES . 'icon_status_red.png', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?>
                </td>
                <td class="dataTableContent" align="center">
<?php
      if ($specifications['show_filter'] == 'True') {
        echo tep_image (DIR_WS_IMAGES . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_filters_spec&flag=0&sgpath=' . $specs_group_path . '&spid=' . $specifications['specifications_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_filters_spec&flag=1&sgpath=' . $specs_group_path . '&spid=' . $specifications['specifications_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image (DIR_WS_IMAGES . 'icon_status_red.png', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?>
                </td>
                <td class="dataTableContent" align="center">
<?php
      if ($specifications['show_feeder'] == 'True') {
        echo tep_image (DIR_WS_IMAGES . 'icon_status_green.png', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_feeder_spec&flag=0&sgpath=' . $specs_group_path . '&spid=' . $specifications['specifications_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_red_light.png', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'action=flag_feeder_spec&flag=1&sgpath=' . $specs_group_path . '&spid=' . $specifications['specifications_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_status_green_light.png', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image (DIR_WS_IMAGES . 'icon_status_red.png', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?>
                </td>
...
<?php
        if (isset ($sInfo) && is_object ($sInfo) && ($specifications['specification_description_id'] == $sInfo->specification_description_id) ) {
          ...
          echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'spath=' . $specs_path . '&spid=' . $specifications['specification_description_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a>';
        }
?>
                &nbsp;</td>
              </tr>
             ...
             <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td align="right" class="smallText"><?php echo tep_draw_button(IMAGE_BACK, 'triangle-1-w', tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $specs_group_path), 'secondary') . '&nbsp;' . tep_draw_button(IMAGE_NEW_SPECIFICATION, 'triangle-1-e', tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&action=new_specification'), 'primary'); ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
              <tr>
              ...
              <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  ...
                  <tr>
                    <td class="smallText"><?php echo tep_image (DIR_WS_ICONS . 'folder.png', ICON_FOLDER) . ' ' . TEXT_SPECS_LEGEND_FILTERS; ?>&nbsp;</td>
                  </tr>
                  <tr>
...
<?php
        if (isset ($fInfo) && is_object ($fInfo) && ($filters_array['specification_filters_id'] == $fInfo->specification_filters_id) ) {
          ...
          echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&fid=' . $filters_array['specification_filters_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a>';
        }
?>
                &nbsp;</td>
              </tr>
             ...
             <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td align="right" class="smallText"><?php echo tep_draw_button(IMAGE_BACK, 'triangle-1-w', tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spid=' . $specs_path), 'secondary') . '&nbsp;' . tep_draw_button(IMAGE_NEW_FILTER, 'triangle-1-e', tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&action=new_filter'), 'primary'); ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
<?php
...
<?php
    ...
    echo tep_draw_hidden_field ('sgpath', $specs_group_path);
    echo tep_draw_hidden_field ('spath', $specs_path);
    echo tep_draw_hidden_field ('svalue', '1');
...
<?php
    ...
    echo tep_draw_hidden_field ('spath', $specs_path);
    echo tep_draw_hidden_field ('svalue', '1');
    echo HEADING_TITLE_GOTO . ' ' . tep_draw_pull_down_menu ('vid', tep_get_specification_values ($specs_path, $languages_id), $svalue_id, 'onClick="this.form.submit();"');
    echo tep_hide_session_id() . '</form>';
?>
                </td>
              </tr>
...
<?php
    ...
    while ($values_array = tep_db_fetch_array ($values_query) ) {
      ...
      if (isset ($vInfo) && is_object ($vInfo) && ($values_array['specification_values_id'] == $vInfo->specification_values_id) ) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&vid=' . $values_array['specification_values_id'] . '&svalue=1') . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&vid=' . $values_array['specification_values_id'] . '&svalue=1') . '\'">' . "\n";
      }
?>
...
<?php
        if (isset ($vInfo) && is_object ($vInfo) && ($filters_array['specification_values_id'] == $vInfo->specification_values_id) ) {
          ...
          echo '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&vid=' . $values_array['specification_values_id'] . '&svalue=1') . '">' . tep_image (DIR_WS_IMAGES . 'icon_info.png', IMAGE_ICON_INFO) . '</a>';
        }
?>
                &nbsp;</td>
              </tr>
             ...
             <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td align="right" class="smallText"><?php echo tep_draw_button(IMAGE_BACK, 'triangle-1-w', tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spid=' . $specs_path), 'secondary') . '&nbsp;' . tep_draw_button(IMAGE_NEW_VALUE, 'triangle-1-e', tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&action=new_value&svalue=1'), 'primary'); ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
<?php
    ...
    switch ($action) {
      case 'new_group':
        ...
        $contents[] = array( 'align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_SAVE, 'disk', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id ) ) );
        break;
      ...
      case 'link_category':
        ...
        $contents[] = array( 'align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_LINK, 'transfer-e-w', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id ) ) );
        break;
      ...
      case 'link_all_categories':
        ...
        $contents[] = array( 'align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_LINK_ALL, 'transferthick-e-w', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id ) ) );
        break;
      ...
      case 'unlink_category':
        ...
        $contents[] = array( 'align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_UNLINK, 'grip-dotted-vertical', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id ) ) );
        break;
      ...
      case 'unlink_all_categories':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_UNLINK_ALL, 'grip-solid-vertical', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id ) ) );
        break;
      ...
      case 'new_specification':
        ...
        $contents[] = array ('text' => '<br>' . TEXT_SHOW_ON_PRODUCTS . '<br>' . tep_draw_radio_field ('show_products', 'True', true) . '&nbsp;' . TEXT_SHOW . '<br>' . tep_draw_radio_field ('show_products', 'False', false) . '&nbsp;' . TEXT_DONT_SHOW);
        $contents[] = array ('text' => '<br>' . TEXT_SHOW_FILTER . '<br>' . tep_draw_radio_field ('show_filter', 'True', true) . '&nbsp;' . TEXT_SHOW . '<br>' . tep_draw_radio_field ('show_filter', 'False', false) . '&nbsp;' . TEXT_DONT_SHOW);
        $contents[] = array ('text' => '<br>' . TEXT_SHOW_FEEDER . '<br>' . tep_draw_radio_field ('show_feeder', 'True', true) . '&nbsp;' . TEXT_SHOW . '<br>' . tep_draw_radio_field ('show_feeder', 'False', false) . '&nbsp;' . TEXT_DONT_SHOW);
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit ('button_save.png', IMAGE_SAVE) . ' <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'spid=' . $specification_id . '&sgpath=' . $specs_group_path) . '">' . tep_image_button ('button_cancel.png', IMAGE_CANCEL) . '</a>');
        break;
      ...
      case 'edit_specification':
        ...
        $contents[] = array ('text' => '<br>' . TEXT_SPECIFICATION_PREFIX . $specification_prefix_string);
        $contents[] = array ('text' => '<br>' . TEXT_SPECIFICATION_SUFFIX . $specification_suffix_string);
        $contents[] = array ('text' => '<br>' . TEXT_EDIT_SORT_ORDER . '<br>' . tep_draw_input_field ('specification_sort_order', $specification_fixed_data['specification_sort_order'], 'size="5"'));
        $contents[] = array ('text' => '<br>' . TEXT_COLUMN_JUSTIFY . '<br>' . tep_draw_pull_down_menu ('column_justify', $alignment_array, $specification_fixed_data['column_justify']) . '<br>');
        $contents[] = array ('text' => '<br>' . TEXT_EXISTING_FIELD . '<br>' . tep_draw_pull_down_menu ('products_column_name', $products_database_fields, $specification_fixed_data['products_column_name']) . '<br>' . TEXT_EXISTING_FIELD_NOTE);
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_SAVE, 'disk', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'spid=' . $specification_id . '&sgpath=' . $specs_group_path ) ) );
        break;
      ...
      case 'move_specification':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_MOVE, 'disk', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'spid=' . $specification_id . '&sgpath=' . $specs_group_path ) ) );
        break;
      ...
      case 'copy_specification':
        ...
        $contents[] = array ('text' => sprintf (TEXT_INFO_COPY_SPECIFICATION_INTRO, $sInfo->specification_name) );
        $contents[] = array ('text' => tep_draw_hidden_field ('specifications_id', $sInfo->specifications_id) . tep_draw_hidden_field ('specification_group_id', $sInfo->specification_group_id) );
        if ($count_filters > 0) $contents[] = array ('text' => tep_draw_checkbox_field ('copy_filter', 'True') . ' ' . sprintf (TEXT_COPY_QUERY_FILTERS, $count_filters) );
        if ($count_values > 0) $contents[] = array ('text' => tep_draw_checkbox_field ('copy_values', 'True') . ' ' . sprintf (TEXT_COPY_QUERY_VALUES, $count_values) );
        $contents[] = array ('text' => '<br>' . TEXT_COPY_SPECIFICATION_TO . '<br>' . tep_draw_pull_down_menu ('group_id', $groups_array, $specs_group_path) );
        $contents[] = array('align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_COPY, 'disk', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'spid=' . $specification_id . '&sgpath=' . $specs_group_path ) ) );
        break;
...
//Filters
      case 'new_filter':
        ...
        if ($check_data['products_column_name'] == 'manufacturers_id') {
          $contents[] = array ('text' => '<br><a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&fid=' . $fInfo->specification_filters_id . '&action=import_manufacturers') . '">' . tep_image_button ('button_import_manufacturers.png', IMAGE_IMPORT_MANUFACTURERS) . '</a>');
        }
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_SAVE, 'disk', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path ) ) );
        break;
      ...
      case 'edit_filter':
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_SAVE, 'disk', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&fid=' . $vInfo->specification_filters_id ) ) );

        $import = '';
        if ($check_data['products_column_name'] == 'manufacturers_id') {
          $import = tep_draw_button( IMAGE_IMPORT_MANUFACTURERS, 'wrench', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&fid=' . $vInfo->specification_filters_id ) );
        }

        $contents[] = array('align' => 'center', 'text' => $import .
          tep_draw_button( IMAGE_SORT, 'gear', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&fid=' . $vInfo->specification_filters_id ) ) );
        break;
...
// Specification Values
      case 'new_value':
        ...
        $contents = array ('form' => tep_draw_form ('new_value', FILENAME_PRODUCTS_SPECIFICATIONS, 'action=new_value_confirm&sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1', 'post', 'enctype="multipart/form-data"') );
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_SAVE, 'disk', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1' ) ) );
        break;

      case 'edit_value':
        $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_EDIT_VALUE . '</b>');

        $contents = array('form' => tep_draw_form ('edit_value', FILENAME_PRODUCTS_SPECIFICATIONS, 'action=edit_value_confirm&sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1', 'post', 'enctype="multipart/form-data"') );
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_SAVE, 'disk', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1&vid=' . $vInfo->specification_values_id ) ) );
        break;

      case 'delete_value':
        $heading[] = array ('text' => '<b>' . TEXT_INFO_HEADING_DELETE_VALUE . '</b>');

        $contents = array ('form' => tep_draw_form ('delete_value', FILENAME_PRODUCTS_SPECIFICATIONS, 'action=delete_value_confirm&sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1') );
      ...
      case 'move_value':
        ...
        $contents = array ('form' => tep_draw_form ('move_value', FILENAME_PRODUCTS_SPECIFICATIONS, 'action=move_value_confirm&sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1') );
      ...
      case 'copy_value':
        ...
        $contents = array ('form' => tep_draw_form ('copy_value', FILENAME_PRODUCTS_SPECIFICATIONS, 'action=copy_value_confirm&sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1') );
        $contents[] = array ('text' => sprintf (TEXT_COPY_VALUE_INTRO, $vInfo->value) );
        $contents[] = array ('text' => tep_draw_hidden_field ('specification_values_id', $fInfo->specification_values_id) );
        $contents[] = array ('text' => sprintf (TEXT_COPY_VALUE_TO, $vInfo->value) . '<br>' . tep_draw_pull_down_menu ('copy_to_value_id', $specifications_list, $specs_path) );
        $contents[] = array('align' => 'center', 'text' => '<br />' .
          tep_draw_button( IMAGE_COPY, 'disk', null, 'primary' ) .
          tep_draw_button( IMAGE_CANCEL, 'close', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&svalue=1&vid=' . $vInfo->specification_values_id ) ) );
        break;
          ...
          case ($specs_group_path == 0 && $specs_path == 0):
            if ($rows > 0) {
              if (isset ($cInfo) && is_object ($cInfo)) { // Specification groups info box contents
				...
				'<a href="' . tep_href_link(FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id . '&action=edit_group') . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a>
				<a href="' . tep_href_link(FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id . '&action=delete_group') . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a>
				<a href="' . tep_href_link(FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id  . '&action=copy_group') . '">' . tep_image_button('button_move.png', IMAGE_MOVE) . '</a>');

                $contents[] = array('align' => 'center', 'text' =>
                  tep_draw_button( IMAGE_LINK, 'transfer-e-w', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id . '&action=link_category' ) ) .
                  tep_draw_button( IMAGE_LINK_ALL, 'transferthick-e-w', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id . '&action=link_all_categories' ) ) .
                  tep_draw_button( IMAGE_UNLINK, 'grip-dotted-vertical', tep_href_link( FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id . '&action=unlink_category' ) ) .
                  tep_draw_button( IMAGE_UNLINK_ALL, 'grip-solid-vertical', tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgid=' . $cInfo->specification_group_id . '&action=unlink_all_categories') ) );
          ...
          case ($specs_group_path != 0 && $specs_path == 0):
            if ($rows > 0) {
              if (isset ($sInfo) && is_object ($sInfo)) { // Specification info box contents
                ...
                $contents[] = array ('align' => 'center', 'text' => '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spid=' . $sInfo->specifications_id . '&action=edit_specification') . '">' . tep_image_button ('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spid=' . $sInfo->specifications_id . '&action=delete_specification') . '">' . tep_image_button ('button_delete.png', IMAGE_DELETE) . '</a> <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spid=' . $sInfo->specifications_id . '&action=copy_specification') . '">' . tep_image_button ('button_copy.png', IMAGE_COPY) . '</a>  <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spid=' . $sInfo->specifications_id . '&action=move_specification') . '">' . tep_image_button ('button_move.png', IMAGE_MOVE) . '</a>');
          ...
          case ($specs_path != '0' && $svalue != '1'):
            if ($rows > 0) {
              if (isset ($fInfo) && is_object ($fInfo)) { // Filters info box contents
                ...
                $contents[] = array ('align' => 'center', 'text' => '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&fid=' . $fInfo->specification_filters_id . '&action=edit_filter') . '">' . tep_image_button ('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&fid=' . $fInfo->specification_filters_id . '&action=delete_filter') . '">' . tep_image_button ('button_delete.png', IMAGE_DELETE) . '</a> <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&fid=' . $fInfo->specification_filters_id . '&action=copy_filter') . '">' . tep_image_button ('button_copy.png', IMAGE_COPY) . '</a> <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&fid=' . $fInfo->specification_filters_id . '&action=move_filter') . '">' . tep_image_button ('button_move.png', IMAGE_MOVE) . '</a>');
          ...
          case ($specs_path != '0' && $svalue == '1'):
            if ($rows > 0) {
              if (isset ($vInfo) && is_object ($vInfo)) { // filter values info box contents
                ...
                $contents[] = array ('align' => 'center', 'text' => '<a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&vid=' . $vInfo->specification_values_id . '&svalue=1&action=edit_value') . '">' . tep_image_button ('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&vid=' . $vInfo->specification_values_id . '&svalue=1&action=delete_value') . '">' . tep_image_button ('button_delete.png', IMAGE_DELETE) . '</a> <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&vid=' . $vInfo->specification_values_id . '&svalue=1&action=copy_value') . '">' . tep_image_button ('button_copy.png', IMAGE_COPY) . '</a> <a href="' . tep_href_link (FILENAME_PRODUCTS_SPECIFICATIONS, 'sgpath=' . $specs_group_path . '&spath=' . $specs_path . '&vid=' . $vInfo->specification_values_id . '&svalue=1&action=move_value') . '">' . tep_image_button ('button_move.png', IMAGE_MOVE) . '</a>');

...
if (is_array($custid) && count($custid) > 0 ) {  ?>
            <tr>
	...
	unset($email);
      ...
      $cline .= "<tr class='dataTableRow'>
		...
		if( EMAIL_USE_HTML == 'true' )
			...
			$mline .= '   (' . tep_catalog_href_link(FILENAME_CATALOG_PRODUCT_INFO, 'products_id='. $inrec['pid']).")</td></tr></table>\n";
	  }
	  ...
	  $email .= EMAIL_TEXT_BODY_HEADER . $mline . EMAIL_TEXT_BODY_FOOTER;
		...
		if( EMAIL_USE_HTML == 'true' )
			$email .= EMAIL_TEXT_LOGIN . "\n" .'<a HREF="' . tep_catalog_href_link(FILENAME_LOGIN, '', 'NONSSL') . '">' . tep_catalog_href_link(FILENAME_LOGIN, '', 'NONSSL')  . '</a>';
		else
			$email .= '  (' . tep_catalog_href_link(FILENAME_LOGIN, '', 'NONSSL') . ')';

		$email .= "\n" . EMAIL_SEPARATOR . '</td></tr></table>'."\n\n";

		//$email .= "\n\n". $_POST['message']; llevado a languages
...
<?php
 ...
 for ($i = 0; $i <= $knt; $i++)
 {
   $inrec = tep_db_fetch_array($query1);
    ...
    if (is_array($inrec) && $curcus != $inrec['cid'])
    {
      // output line
      $totalAll += $tprice;
      $cline .= "       </td>

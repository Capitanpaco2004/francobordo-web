<?php
/**
 * cron_stock_kent.php — actualizador de stock + descatalogados KENT / Alliance Marine (CLI, para cron)
 *
 * Fuente: SFTP Alliance Marine products/KENT_products_francobordo.csv (se regenera cada ~30 min;
 ...
 *   - En feed con stock KENT > 0 (Active o End of life) → -100 (en proveedor).
 *   - En feed Active sin stock KENT               → -800 (bajo pedido).
 ...
 *     en kent_csv_descatalogados. Si REAPARECE disponible → status 2→1 y se desanota (solo los que
 ...
 *
 * Uso:
 *   php cron_stock_kent.php                     # DRY: reporta sin tocar
 *   php cron_stock_kent.php --apply             # ejecuta (lo usa el cron)
 *   php cron_stock_kent.php --skip-download     # usa la copia local del CSV
 */

$apply        = in_array('--apply', $argv ?? [], true);
$skipDownload = in_array('--skip-download', $argv ?? [], true);

const CSV_PATH    = '/home/francobordo/public_html/import/feed/KENT/KENT_products_francobordo.csv';
...
const SFTP_REMOTE = 'products/KENT_products_francobordo.csv';
const STOCK_PROVEEDOR   = -100; // KENT tiene stock
const STOCK_BAJOPEDIDO  = -800; // en catálogo sin stock
const STOCK_DESCATALOG  = -900; // fuera de catálogo / EOL agotado
const MARKER_TABLE = 'kent_csv_descatalogados';
...
function targetFor($rec) {
    ...
    if ($rec['stock'] > 0) return STOCK_PROVEEDOR;           // KENT tiene stock (Active o EOL)
...
// ── productos KENT + variantes + stock por variante ──
$prods = [];
$res = $db->query("SELECT products_id, products_model, reference_prov, product_ean, products_quantity, products_status FROM products WHERE products_import_origin LIKE 'kent%'");
if (!$res) { out('ERROR query productos: ' . $db->error); exit(1); }
while ($p = $res->fetch_assoc()) $prods[(int) $p['products_id']] = $p;
if (empty($prods)) { out('No hay productos origin=kent en BD. Nada que hacer.'); exit(0); }
...
out("[$mode] BD KENT: $nProds productos | marcador: " . count($marker));

<?php
/**
 * cron_stock_seimi.php — actualizador de stock + descatalogados SEIMI / Alliance Marine (CLI, para cron)
 *
 * Fuente: SFTP Alliance Marine products/SEIMI_products_francobordo.csv (se regenera cada ~30 min;
 ...
 *   - En feed con stock SEIMI > 0 (Active o End of life) → -100 (en proveedor).
 *   - En feed Active sin stock SEIMI               → -800 (bajo pedido).
 ...
 *     en seimi_csv_descatalogados. Si REAPARECE disponible → status 2→1 y se desanota (solo los que
 ...
 *
 * Uso:
 *   php cron_stock_seimi.php                     # DRY: reporta sin tocar
 *   php cron_stock_seimi.php --apply             # ejecuta (lo usa el cron)
 *   php cron_stock_seimi.php --skip-download     # usa la copia local del CSV
 */

$apply        = in_array('--apply', $argv ?? [], true);
$skipDownload = in_array('--skip-download', $argv ?? [], true);

const CSV_PATH    = '/home/francobordo/public_html/import/feed/SEIMI/SEIMI_products_francobordo.csv';
...
const SFTP_REMOTE = 'products/SEIMI_products_francobordo.csv';
const STOCK_PROVEEDOR   = -100; // SEIMI tiene stock
const STOCK_BAJOPEDIDO  = -800; // en catálogo sin stock
const STOCK_DESCATALOG  = -900; // fuera de catálogo / EOL agotado
const MARKER_TABLE = 'seimi_csv_descatalogados';
...
function targetFor($rec) {
    ...
    if ($rec['stock'] > 0) return STOCK_PROVEEDOR;           // SEIMI tiene stock (Active o EOL)
...
// ── productos SEIMI + variantes + stock por variante ──
$prods = [];
$res = $db->query("SELECT products_id, products_model, reference_prov, product_ean, products_quantity, products_status FROM products WHERE products_import_origin LIKE 'seimi%'");
if (!$res) { out('ERROR query productos: ' . $db->error); exit(1); }
while ($p = $res->fetch_assoc()) $prods[(int) $p['products_id']] = $p;
if (empty($prods)) { out('No hay productos origin=seimi en BD. Nada que hacer.'); exit(0); }
...
out("[$mode] BD SEIMI: $nProds productos | marcador: " . count($marker));

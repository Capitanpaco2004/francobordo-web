...
// detecte les constantes ou initialise
  $today = getdate();
    if ($_GET['year']) $year = (int)tep_db_prepare_input($_GET['year']); 
    else $year = $today['year']; //  else $year = 'ALL';
    if ($_GET['month']) $month = (int)tep_db_prepare_input($_GET['month']); 
    else $month = $today['mon']; //  else $month = 'ALL';
    if ($_GET['mini_ordered']) $mini_ordered = (int)tep_db_prepare_input($_GET['mini_ordered']); 
    else $mini_ordered = 1;
    if ($_GET['no_status']) $no_status = (int)tep_db_prepare_input($_GET['no_status']); 
    if ($_GET['status']) $status = (int)tep_db_prepare_input($_GET['status']); 

  $list_year_array = array();
...
//  get list of month for dropdown selection
  $list_month = array(JAN, FEV, MAR, AVR, MAI, 'Junio', JUI, AOU, SEP, OCT, NOV, DEC);
...
<!-- body_text //-->
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td><?php echo tep_draw_form('search', FILENAME_STATS_CUSTOMERS_ORDERS, '', 'get'); ?><table border="0" cellspacing="0" cellpadding="2">
          <tr>

...
// ********** PAY PER CLICK CONFIGURATION SECTION EOF ************
  ...
  // $contribPath = 'http://addons.oscommerce.com/info/3924';
  // $currentVersion = 'Supertracker 3.5 for OSC 2.2.';
  // $contribName = 'Supertracker';
  /********************** END VERSION CHECKER *********************/
...
?>
                                                  <?php echo TABLE_TEXT_MENU_DESC_TEXT; ?> <form name="report_select"><select name="report_selector" onChange="page_redirect(this)">
                                                                ...
                                                                <option value="supertracker.php?report=browser"><?php echo TEXT_BROWSER;?></option>
                                                                ...
                                                                <option value="supertracker.php?special=state"><?php echo TEXT_VISITORS_STATE;?></option>
                                                                <option value="supertracker.php?special=city"><?php echo TEXT_VISITORS_CITY;?></option>
...
<?php
                 while ($lt_row = tep_db_fetch_array($lt_result)) {
                   $customer_ip = $lt_row['ip_address'];
                         ...
                         $country_name = mb_convert_encoding($lt_row['country_name'] ?? '', 'UTF-8', 'ISO-8859-1');
                         $region_name = mb_convert_encoding($lt_row['country_region'] ?? '', 'UTF-8', 'ISO-8859-1');
                         $city_name = mb_convert_encoding($lt_row['country_city'] ?? '', 'UTF-8', 'ISO-8859-1');
      ...
      if($country_code==''){
        $country_code='pixel_trans';
            ...
            echo '<table width="100%" border=0 cellspacing=0 cellpadding=5 style="border:1px solid #000;">';
						echo '<tr><td class="dataTableContent"><b>' . TABLE_TEXT_IP . '</b><a href="http://www.showmyip.com/?ip=' . $customer_ip . '" target="_blank">' . $customer_ip . ' (' . $country_name . ') ' . tep_image(DIR_WS_IMAGES . 'geo_flags/flags/' . $country_code . '.gif') . ' ' . gethostbyaddr($customer_ip) . '</td></tr>';
                  ...
                  $categories_viewed = unserialize($lt_row['categories_viewed']);
                        if (!empty($categories_viewed)) {
                          ...
                          foreach ($categories_viewed as $cat_id=>$val) {
                                  $cat_query = "select * from categories as c, categories_description as cd where c.categories_id=cd.categories_id and c.categories_id='" . $cat_id . "' and cd.language_id = '" . (int)$languages_id . "'";
                        ...
                        if ($products_viewed != '') {
                          ...
                          foreach ($prod_view_array as $key=>$product_id) {
                                  $product_id = rtrim($product_id, '?');
                                        if ($product_id != '') {
                      $prod_query = "select * from products as p, products_description as pd where p.products_id=pd.products_id and p.products_id='" . $product_id . "' and pd.language_id = '" . (int)$languages_id . "'";
...
<?php
           ...
           $country_names[$country_code] = tep_image(DIR_WS_IMAGES . 'geo_flags/flags/' . $country_code . '.gif') . ' ' . $geo_row['country_name'];
                 }
                 draw_geo_graph($geo_hits,$country_names,$total_hits);
         }//End Geo Report

   if ($_GET['special'] == 'state') {
?>
         <tr>
                <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
<?php
           ...
           $country_names[$country_code] = $state_row['country_region'];
                 ...
                 draw_geo_graph($state_hits,$country_names,$total_hits);
         }//End Geo Report

   if ($_GET['special'] == 'city') {
?>
         <tr>
                <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
<?php
           ...
           $country_names[$country_code] = $city_row['country_city'];
                 ...
                 draw_geo_graph($city_hits,$country_names,$total_hits);
         }//End Geo Report
 }
?>

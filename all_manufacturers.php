<?php
	...
	if( isset( $_GET['search'] ) && $_GET['search'] != '' )
	{
		...
		// Formateamos las palabras de la b�squeda
		foreach( $aWords as $aWord )
		{
			// Eliminamos las palabras demasiado cortas (pronombres, etc)
			if( strlen( $aWord ) > 1 )
			{
				// Si es una preposici�n, art�culo o nexo continuamos
				if( in_array( $aWord, array( 'a', 'ante', 'bajo', 'cabe', 'con', 'contra', 'de', 'desde', 'en', 'entre', 'hacia', 'hasta', 'para', 'por', 'seg�n', 'segun', 'sin', 'so', 'sobre', 'tras', 'del', 'al', 'el', 'la', 'las', 'los', 'un', 'unos', 'un', 'una', 'unas', 'y', 'u', 'o', 'e' ) ) )
					continue;

				// Si es un n�mero
				if( is_numeric( $aWord ) )
				{
					// A�adimos el n�mero en singular y plural
					$aSearchPlural[] = $aWord;
					$aSearchSingular[] = $aWord;
					continue;
				}

				// Comprobamos si la palabra est� en plural (si termina en -s, -es, -ces), para obtener su singular
				if( preg_match( '/s$/', $aWord ) || preg_match( '/es$/', $aWord ) || preg_match( '/ces$/', $aWord ) )
				{
					// A�adimos al array plural
					$aSearchPlural[] = $aWord;

					// A�adimos al array singular
		...
		// Recorremos las palabras y las a�adimos al WHERE para el nombre

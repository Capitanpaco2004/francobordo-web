...
/* Ejecuta la consulta con el filtro dado y construye las filas del PDF. */
$fetchRows = function ($whereExtra, $bindVal) use ($db, $selCols) {
    $st = $db->prepare($selCols . $whereExtra . " ORDER BY s.id ASC");
    if (!$st) { error_log('cex_manifiesto prepare: ' . $db->error); out_json(array('ok' => false, 'error' => 'consulta no disponible')); }
    if ($bindVal !== null) $st->bind_param('s', $bindVal);
    $st->execute();
    $res = $st->get_result();
    ...
    return array($rows, $ids, $totBultos, $totKg);
};

/* Timestamp del último manifiesto emitido (para reimpresión). */
$maxLoteTs = function () use ($db) {
...
if ($legacyDay !== '') {
    ...
    list($rows, $ids, $totBultos, $totKg) = $fetchRows(" AND DATE(s.date_added)=?", $legacyDay);
} elseif ($reprintLast) {
    /* Reimpresión explícita del último lote emitido. */
    $mMax = $maxLoteTs();
    if ($mMax === '') out_json(array('ok' => false, 'error' => 'No hay ningun manifiesto emitido todavia.'));
    list($rows, $ids, $totBultos, $totKg) = $fetchRows(" AND s.manifested_at = ?", $mMax);
} else {
    /* MODO PENDIENTES (por defecto): todo lo preparado que no salió en manifiestos anteriores. */
    list($rows, $ids, $totBultos, $totKg) = $fetchRows(" AND s.manifested_at IS NULL", null);
    if (!$rows && !$preview) {
        /* Nada pendiente (ya se emitió todo lo preparado): en vez de "no hay envíos", REIMPRIME
         * el último manifiesto emitido, para que el operario SIEMPRE obtenga un PDF útil desde
         * el .bat (que solo muestra un genérico "no hay manifiesto" ante el JSON de error).
         * No re-marca: se comporta como reprint_last (banner REIMPRESIÓN). El preview NO
         * reimprime: informa honestamente de que no queda nada pendiente. */
        $mMax = $maxLoteTs();
        if ($mMax !== '') {
            $reprintLast = true;
            list($rows, $ids, $totBultos, $totKg) = $fetchRows(" AND s.manifested_at = ?", $mMax);
...
if (!$rows) {
    ...
    if ($preview)          out_json(array('ok' => false, 'error' => 'Nada pendiente de manifiesto (todo lo preparado ya se emitió).'));
    out_json(array('ok' => false, 'error' => 'No se ha emitido ningun manifiesto Correos Express todavia.'));
}

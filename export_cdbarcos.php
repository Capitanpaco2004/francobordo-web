...
if( ($verif == true and $p==$pass) OR $verif == false ) 
  ...
  $products['precio_inicial']= $products['products_price'];
 ...
 if (intval($products['categories_id']) > 0) {
          $categories_query = tep_db_query("SELECT categories_cdbarcos, categories_name FROM " . TABLE_CATEGORIES_DESCRIPTION . " WHERE categories_id = " . $products['categories_id'] . " AND language_id= FLOOR($languages_id)" );
          $categories_result = tep_db_fetch_array($categories_query);
		  if($categories_result['categories_cdbarcos'] == 0 )   {
          $products['categories_cdbarcos'] =11;
	  $products['categories_name']=$categories_result['categories_name'];
	  		  } else {
 $products['categories_cdbarcos'] = $categories_result['categories_cdbarcos'];
 $products['categories_name']=$categories_result['categories_name'];
 }
 }

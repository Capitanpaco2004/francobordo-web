...
// if the customer is not logged on, redirect them to the login page
  if (!tep_session_is_registered('customer_id')) {
    $snapshot = array('page' => 'ext/modules/payment/paypal/paypal_express.php',
                      'mode' => $request_type,
                      'get' => $_GET,
                      'post' => $_POST);
	...
	$sUrl = str_replace( 'ext/modules/payment/paypal/', '', tep_href_link(FILENAME_LOGIN, '', 'SSL') );
...
// if there is nothing in the customers cart, redirect them to the shopping cart page
  if ($cart->count_contents() < 1) {
	$sUrl = str_replace( 'ext/modules/payment/paypal/', '', tep_href_link(FILENAME_SHOPPING_CART) );
...
*/
          ...
          $ppe_payerid = $response_array['PAYERID'];
			...
			if ( key_exists( 'origin', $_SESSION ) && $_SESSION['origin'] == 'appcommerce' ) 
			{
				$sUrl = str_replace( 'ext/modules/payment/paypal/', '', tep_href_link(FILENAME_CHECKOUT_CONFIRMATION_APPC) );
	...
	// die();
			...
			{
				$sUrl = str_replace( 'ext/modules/payment/paypal/', '', tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, '', 'SSL') );
	// echo $sUrl . '5';
	// die();
				tep_redirect( $sUrl );
			}
	...
	// die();
	  ...
	  {
		$sUrl = str_replace( 'ext/modules/payment/paypal/', '', tep_href_link(FILENAME_SHOPPING_CART, 'error_message=' . stripslashes($response_array['L_LONGMESSAGE0']), 'SSL') );
	// echo $sUrl . '8';
	// die();
        tep_redirect( $sUrl );
      }
    ...
    default:	
      ...
      $params['RETURNURL'] = str_replace( 'express.php', 'paypal_express.php', tep_href_link('express.php', 'osC_Action=retrieve', 'SSL', true, false) );
										
      $params['CANCELURL'] = str_replace( 'ext/modules/payment/paypal/', '', tep_href_link(FILENAME_SHOPPING_CART, '', 'SSL', true, false) );
      ...
      if (($response_array['ACK'] == 'Success') || ($response_array['ACK'] == 'SuccessWithWarning')) {
		$sUrl = str_replace( 'ext/modules/payment/paypal/', '', $paypal_url . '&token=' . $response_array['TOKEN'] );	
	// echo $sUrl . '9';
	// die();
        tep_redirect( $sUrl );
      } else {
	...
	// die();
      ...
      break;
		...
		$sUrl = str_replace( 'ext/modules/payment/paypal/', '', tep_href_link(FILENAME_SHOPPING_CART, '', 'SSL') );
	// echo $sUrl . '11';
	// die();
        tep_redirect( $sUrl );

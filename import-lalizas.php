...
// Constantes
const FEED_URL = 'https://www.lalizas.com/assets/SPAIN_33.xml';
...
function fetchProducts()
{
    $result = tep_db_query(
        'SELECT products_id, products_quantity, LCASE(products_model) AS products_model 
         FROM products 
         WHERE manufacturers_id IN (3, 96, 16, 246, 376) AND products_status = 1;'
    );
...
function fetchAttributeProducts()
{
    $result = tep_db_query(
        'SELECT pa.products_id, pa.options_id, pa.options_values_id, LCASE(pa.reference) AS reference 
         ...
         WHERE p.manufacturers_id IN (3, 96, 16, 246, 376) AND p.products_status = 1;'
    );
...
function updateAttributeStock($model, $stock, $allAtriProducts)
{
    ...
    $currentStock = $row ? (int) $row['products_stock_quantity'] : 0;

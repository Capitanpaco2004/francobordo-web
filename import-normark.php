...
$fFile = fopen( $sServer . '/descargas/normark.csv', "r");
...
// Email informativo
//mail( 'victor.ruiz@denox.es', '[CORRECTO] Importador de stock Normark a Francobordo', 'Se ha actualizado con exito la tienda Francobordo.', 'MIME-Version: 1.0' . "\r\n" . 'Content-type: text/plain; charset=UTF-8' . "\r\n" . 'From:info@francobordo.com' . "\r\n" );
...
function addProduct($aValues, $aDatabase)
{
	...
	if( tep_db_num_rows( $aProducts ) > 0 )
	{
		...
		if( $aProducts['products_quantity'] <= 0 && $nStock > 2 )
			$aValues['products_quantity'] = -100;
		elseif( $aProducts['products_quantity'] <= 0 && $nStock <= 1 )
	...
	if( tep_db_num_rows( $aProducts ) > 0 )
	{
		...
		if( $aStockAtri['products_stock_quantity'] <= 0 && $nStock > 1 )
			$nStock = -100;
		elseif( $aStockAtri['products_stock_quantity'] <= 0 && $nStock <= 1 )
			$nStock = -900;
		else

...
class PriceFormatter {
  ...
  public $taxClass;
  public $qtyBlocks;
  public $products_min_order_qty;
  public $price_breaks;
  public $hasQuantityPrice;
  public $hiPrice;
  public $lowPrice;
  public $hasSpecialPrice;
  public $specialPrice;
  public $cg_id;
  public $product_id;
  public $discount_categories_id;
  ...
  function loadProduct($product_id, $language_id = 3, $listing = NULL, $price_breaks_from_listing = NULL) {
    global $pfs;
    $this->cg_id = $this->get_customer_group_id();

	$nProduct_id = $product_id;
  ...
  if (!isset($price_formatter_data)) {
    if ($listing == NULL) {
      //Collect required data
       $sql = "select pd.products_name, p.products_model, p.products_ubicacion, p.products_image, p.products_id," .
...
// customer group price can be null when qty blocks or min order qty is used
  ...
  // now get the specials price for this customer_group and add it to product_info array
		$specials_query = tep_db_query("select s.products_id, s.specials_new_products_price
										from " . TABLE_SPECIALS . " s
										where s.products_id = " . (int)$product_id . "
										and s.status = '1'
										...
										and s.specials_new_products_price IS NOT NULL
										AND (s.venta_flash = 0 OR s.venta_flash = 1 AND NOW() >= s.start_date)
										and s.customers_group_id = '" . $this->cg_id . "'");

		// $specials_query = tep_db_query("select products_id, specials_new_products_price
										// from " . TABLE_SPECIALS . "
										// where products_id = " . (int)$product_id . " and status = '1'
										// and specials_new_products_price IS NOT NULL AND (venta_flash = 0 OR venta_flash = 1 AND NOW() >= start_date)
										// and customers_group_id = '" . $this->cg_id . "'");

  if ($specials_price = tep_db_fetch_array($specials_query)) {
      ...
      $price_formatter_data = array(
		...
		'products_ubicacion' => $product_info['products_ubicacion'],
      ...
      $price_formatter_data = array(
		...
		'products_ubicacion' => $listing['products_ubicacion'],
	...
	if( !array_key_exists( 'curl_oe', $_GET ) )
	{
		...
		// Guardamos id de producto
		foreach( $aAux as $key => $value )
			$aIds[] = preg_replace( '/\{.+$/i', '', $key );

		while( $aDato = tep_db_fetch_array( $aDatos ) )
		{
			if( in_array( $aDato['parent_id'], $aIds ) )
			{
				// Separamos, id producto, option id y option value para obtener el precio del atributo
				preg_match( '/(?P<products_id>\d+){(?P<options_id>\d+)}(?P<options_values_id>\d+)/i', $nProduct_id, $aMatch );

				// Precio del atributo
				$nPrecioAtributo = tep_db_query( 'SELECT options_values_price FROM products_attributes WHERE products_id = "' . (int)$aMatch['products_id'] . '" AND options_id = "' . (int)$aMatch['options_id'] . '" AND options_values_id = "' . (int)$aMatch['options_values_id'] . '"' );
				$nPrecioAtributo = tep_db_fetch_array( $nPrecioAtributo );
				$nPrecioAtributo = $nPrecioAtributo['options_values_price'];

				// Calculamos porcentaje de oferta
				$nPorcentaje = floatval( $aDato['price'] * 100 / $this->thePrice );

				// Calculamos
				$nPrecio = ($nPorcentaje * ($this->thePrice  + $nPrecioAtributo) / 100);
				$this->thePrice = $nPrecio - $nPrecioAtributo;
			}
		}
	}
	// Fin, products_together
  ...
  function computePrice($qty, $nof_other_items_in_cart_same_cat = 0)
  {
	...
	function adjustQty($qty, $qtyBlocks = NULL)
	{
		...
		if ($qtyBlocks >= 1) {
			...
			if (($qty % $qtyBlocks) != 0) {
				...
				if (!array_key_exists('curl_oe', $_GET) && defined('MAX_QTY_IN_CART') && (MAX_QTY_IN_CART > 0) && ($qty > MAX_QTY_IN_CART)) {
					$qty -= $qtyBlocks;
				}
	...
	function getPriceString($style = 'productPriceInBox')
	{
      ...
      <td style="' . $styling . '" class="infoBoxContents">' . TEXT_SAVINGS . '</td>' . "\n";
			...
			$lc_text['products_price'] = $lc_text;
		} else {
			if (true == $this->hasSpecialPrice) {
				$lc_text['products_price'] = $currencies->display_price($this->thePrice, tep_get_tax_rate($this->taxClass));
				$lc_text['products_specials_price'] = $currencies->display_price($this->specialPrice, tep_get_tax_rate($this->taxClass));
			} else {
				$lc_text['products_price'] = $currencies->display_price($this->thePrice, tep_get_tax_rate($this->taxClass));
			}
		}

...
class PriceFormatterStore {
  public $priceFormatterData = array();
  public $cg_id;
...
// customer group price can be NULL so use retail if empty
                   if ((float)$new_prices[$i]['products_price'] > 0 ) {
...
// an extra query is needed for all the specials
       ...
       $specials_query = tep_db_query("select s.products_id, s.specials_new_products_price
									   ...
									   where s.products_id in (" . $product_id_list . ") 
									   and s.status = '1'
		...
		// $specials_query = tep_db_query("select products_id, specials_new_products_price
									   // from " . TABLE_SPECIALS . " 
									   // where products_id in (" . $product_id_list . ") and status = '1'

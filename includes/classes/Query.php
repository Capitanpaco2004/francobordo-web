...
class Query
{
	 ...
	 * @var int|null
	 */
	protected int $countStatement = 0;
    ...
    public function __toString()
    {
        ...
        return str_replace( '`', '', QuerySplitter::bind($this->cachedStatement, $this->params) );
    }
     ...
     *
     * @param int $flags  queryBuild::SPLIT_% and queryBuild::UNQUOTE options
     * @return array
     */
    public function getColumns($flags = 0, $bClear = false)
    {
        // Partes
        $aReturn = querySplitter::splitColumns($this->getParts(), $flags);

        // Recorremos
        foreach( $aReturn as $key => $value )
        {
            // Si contenemos alias
            if( preg_match( '/ AS /i', $value) )
            {
        ...
        if( $bClear )
        {
            ...
            foreach( $aAuxs as $key => $value )
                $aReturn[] = preg_replace( '/^(.+) AS |^(.+)\./i', '', str_replace( '`', '', $value ) );
        }
     ...
     * @return Query  $this
     */
    public function leftOuterJoin($table, $on, $flags = 0)
    {
        return $this->addTable($table, "LEFT OUTER JOIN", $on, $flags);
    }
     ...
     * @return Query  $this
     */
    public function rightOuterJoin($table, $on, $flags = 0)
    {
        return $this->addTable($table, "RIGHT OUTER JOIN", $on, $flags);
    }
    ...
    public function values($values, $flags = 0)
    {
        ...
        if (is_array($values)) {
            foreach ($values as &$value)
                $value = QuerySplitter::quote($value, 'DEFAULT');
            $values = join(', ', $values);
        }

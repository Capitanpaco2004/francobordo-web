...
class QuerySplitter
{
	...
	public static function join($parts)
	{
		...
		foreach ($parts as $key => &$part) {
			...
			if (isset($part) || empty($sql_parts)) {
				...
				$sql_part = $key === 'columns' || $key === 'query' || $key === 'table' || $key === 'options' ? '' : strtoupper($key) . (isset($part) ? " " : "");
				if (isset($part)) {
					$sql_part .= trim($part, " \t\n,");
				}
				$sql_parts[] = $sql_part;
	...
	public static function splitValues($sql, $flags = 0)
	{
        if (is_array($sql) || self::getQueryType($sql)) {
            $parts = is_array($sql) ? $sql : self::split($sql);
			
			$where = preg_replace( '/\(|\)/i', '', $parts['where']);
			$where = explode(' ', $where);
			$return = [];
			$operators = ['=', '!=', '<', '<=', '>', '>='];
			...
			foreach ($where as $word) {
				if ($hasOperator && !in_array(strtolower($word), $conditions))
					$value .= $word;
				...
				if (in_array(strtolower($word), $conditions)) {
					$hasCondition = true;
					$condition = strtolower($word);
				...
				if ($field != '' && $hasOperator && $hasCondition) {
    ...
    public static function splitTables($sql)
    {
		...
		foreach ($matches as $i => &$match) {
			...
			$key = !empty($match['alias']) ? preg_replace('/^(?:AS\s*)?(`?)(.*?)\1\s*$/i', '$2', $match['alias']) : '';
			if (isset($match['name'])) {
				$key = trim($match['name'], ' `');
			}
			if ($key !== '') {

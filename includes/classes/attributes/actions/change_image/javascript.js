(function($)
{
    $.fn.uploadFiles = function()
	{
		...
		var fnDeleteImagen = function(dmThis, dmForm)
		{
        ...
        $(this).each(function()
        {
			...
			// Cuando hacemos click en el boton subir imagen
							...
							for( nCont = 0; f = files[nCont]; nCont++ )
							{
								...
								reader.onload = (function(theFile)
								{
									return function(e)
									{
										...
										sHtml += '<td>' + ['<img width="80" height="80" src="', e.target.result, '" title="', escape(theFile.name), '"/>'].join('') + '</td>';
										sHtml += '<td><input type="hidden" name="images[]" value="' + e.target.result + '"/>' + escape(theFile.name) + '</td>';
										sHtml += '<td style="text-aling: center;"><a class="dlte"><span class="icos-trash" style="float: none; margin: 0px auto; display: block;"></span></a></td>';
										dmTable.append( '<tr>' + sHtml + '</tr>' );

										// Evento para cuando borramos
										dmTable.find(".dlte").unbind().click(function(e)
										{
											// Detenemos evento
											e.stopPropagation();
											
											// Eliminamos
											fnDeleteImagen(this, dmForm);
										});
									};
								})(f);

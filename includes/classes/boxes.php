...
*/
  ...
  class tableBox {
    ...
    function tableBoxCart($contents, $direct_output = false) {
      $tableBox_string = '<table id="wlis-tble"';
        ...
        if (isset($contents[$i][0]) && is_array($contents[$i][0])) {
          for ($x=0, $n2=sizeof($contents[$i]); $x<$n2; $x++) {
            if (isset($contents[$i][$x]['text']) && tep_not_null($contents[$i][$x]['text'])) {
              ...
              $tableBox_string .= (isset( $contents[$i][$x]['data-text'] ) ? ' data-text="' . $contents[$i][$x]['data-text'] . '"' : '') . '>';
...
// class constructor
  ...
  class infoBox extends tableBox {
    function __construct($contents, $css_suffix = '', $infoBoxcellpadding = '0', $contentcellpadding = '3') {
  ...
  class infoBoxHeading extends tableBox {
    function __construct($contents, $left_corner = true, $right_corner = true, $right_arrow = false, $css_suffix = '') {
      $this->table_cellpadding = '0';

	#PR $css_suffix = right/left/etc. to identify differend CSS control for component
      if ($right_arrow == true) {
        /* $right_arrow = '<a href="' . $right_arrow . '">' . tep_image(DIR_WS_IMAGES . 'infobox/arrow_right.gif', ICON_ARROW_RIGHT) . '&nbsp;<u>' . $contents[0]['text']  . '</u></a>'; */
		        $right_arrow = '<a href="' . $right_arrow . '"><u>' . $contents[0]['text']  . '</u></a>';
		$nombre_box = '';      
	  } else {
        $right_arrow = '';
		$nombre_box = $contents[0]['text'];
      }
      $info_box_contents = array();
      $info_box_contents[] = array( array('params' => 'height="14" class="infoBoxHeadingLcorner' . $css_suffix .'"',
                                         'text' => ' '),
						array('params' => 'height="14" class="infoBoxHeading' . $css_suffix .'"',
                                         'text' => $nombre_box . $right_arrow ), 
  ...
  class contentBox_star extends tableBox {
    function __construct($contents) {
      $info_box_contents = array();
      $info_box_contents[] = array('text' => $this->contentBoxContents($contents));
      $this->table_cellpadding = '0';

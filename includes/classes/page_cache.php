...
class page_cache {
	...
	public $cache_dir, $cache_filename, $cache_param, $cache_file, $customer_cart, $cache_compress, $cache_language, $cache_currency, $customer_sid;
	public $debug_output = array();
	public $cache_lifetime = 5;
	public $is_guest = false;
	public $debug_messages = false;
	public $cache_file_exists = false;
	public $cache_file_is_expired = true;
	public $write_cache_file = false;
	public $output_ob = false;
		...
		if ($this->is_guest && ENABLE_PAGE_CACHE =='true') {
			...
			if ( $this->cache_file_exists && !$this->cache_file_is_expired ) {
				...
				if ( !tep_not_null($this->customer_sid) )
				{
					...
					$sHtml = str_replace(array("<%CART_CACHE%>", "?<osCsid>", "&<osCsid>"), array($this->customer_cart, $this->customer_sid, $this->customer_sid), file_get_contents($this->cache_file) );
				}
				else
				{
					# No cookies enabled so let's append the osCsid to every URL
					$sHtml =  str_replace(array("<%CART_CACHE%>", "<osCsid>"), array($this->customer_cart, $this->customer_sid), file_get_contents($this->cache_file) );				
				} 

				if( $sHtml != '' )
				{
					// Si existe chat de soporte
					if( file_exists( DIR_FS_CATALOG . '/soporte/' ) )
					{
						define( 'LIVEZILLA_PATH', DIR_FS_CATALOG . '/soporte/' );
						require( DIR_FS_CATALOG . '/soporte/api.php' );		
						$API = new LiveZillaAPI();

						if( $API->IsOperatorAvailable() )
							$sHtml = str_replace( '<%CHAT_CACHE%>', '<a id="chat" href="javascript:void(window.open(\'' . HTTP_SERVER . '/soporte/chat.php\',\'\',\'width=590,height=610,left=0,top=0,resizable=yes,menubar=no,location=no,status=yes,scrollbars=yes\'))"></a>', $sHtml );
						else
							$sHtml = str_replace( '<%CHAT_CACHE%>', '', $sHtml );
					}

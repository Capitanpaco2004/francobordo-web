...
//  $filename -  The name of the file to write to.
  function write_cache(&$var, $filename) {
    $filename = $_SERVER['DOCUMENT_ROOT'] . DIR_FS_CACHE . $filename;
    $success = false;
  ...
  $customer_group_id = $_SESSION['sppc_customer_group_id'];
   ...
   $customer_group_id = '0';
    ...
    if (isset($_GET['products_id']) && is_numeric($_GET['products_id'])) {
      if (($refresh == true) || !read_cache($cache_output, 'also_purchased-' . $language . '-cg' . $customer_group_id . '.cache' . $_GET['products_id'], $auto_expire)) {
        include(DIR_WS_COMPONENTS . FILENAME_ALSO_PURCHASED_PRODUCTS);
        $cache_output = $sAlso;
        write_cache($cache_output, 'also_purchased-' . $language . '-cg' . $customer_group_id. '.cache' . $_GET['products_id']);
      }
    }

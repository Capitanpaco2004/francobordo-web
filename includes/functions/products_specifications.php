<?php
/*
  $Id: products_specifications.php v1.1 20101027 kymation $
...
*/
  ...
  ////
  // URL decode a variable (Disabled)
...
//      return rawurldecode ($variable);
  ...
  function tep_get_filter_sql ($filter_class, $specifications_id, $filter_array, $products_column_name, $languages_id) {
  ...
  function tep_get_filter_string ($display_type, $filters_select_array, $target, $filter_name, $filter_value) {
    ...
    switch ($display_type) {
      case 'pulldown':
        ...
        $box_text .= '<noscript>' . tep_image_submit('icon_next.gif', 'Encontrar productos') . '</noscript>';
        $box_text .= '</form>';
        break;
      ...
      case 'multi':
        ...
        $box_text .= $additional_variables . tep_hide_session_id();
        $box_text .= tep_image_submit ('icon_next.gif', 'Encontrar productos');
        $box_text .= '</form>';
        break;
...
$cPath_new = tep_get_product_path( $columns_array['products_id'] );
    ...
    $field_array['products_image'] = '<a href="' . $product_link . '">' . tep_image(DIR_WS_IMAGES . '/productos/' . $columns_array['products_image'], $columns_array['products_name'], 50, 50) . '</a>';
    ...
    $field_array['buy_now'] = tep_draw_hidden_field('products_id', $columns_array['products_id']) . tep_draw_button(IMAGE_BUTTON_IN_CART, 'cart', null, 'primary');

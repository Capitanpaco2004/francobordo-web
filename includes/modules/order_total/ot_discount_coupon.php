...
class ot_discount_coupon {
	public $title, $output, $coupon;
	public $code = 'ot_discount_coupon';
	public $enabled;
	public $sort_order;
	public $_check;
	...
	public function process() {
		...
		if (is_object($order->coupon)) {
			...
			if (MODULE_ORDER_TOTAL_DISCOUNT_COUPON_DISPLAY_LINES == 'false') {
				$discount_lines = [array_sum($order->coupon->applied_discount)];
			} else {
				$discount_lines = $order->coupon->applied_discount;
			}

			if (is_array($discount_lines)) {
				foreach ($discount_lines as $tax_group => $discount) {
					if ($discount > 0) {
						//add in the tax if needed:
						if (MODULE_ORDER_TOTAL_DISCOUNT_COUPON_DISPLAY_TAX == 'Display discount with discounted tax applied' && is_array($order->coupon->discount_tax)) {
						...
						// Descomponer el cupon con ivas
						$nTotalSinTax = 0;
						$nTotalConTax = 0;

						// Recorremos los descuentos por tax
						foreach ($order->coupon->applied_discount as $sTax => $sValue) {
							// Obtenemos el iva
							$sTaxAux = preg_replace('/[^0-9]/', '', $sTax);
							$sTaxAux = 1 + (floatval($sTaxAux) / 100);

							// Descontamos al cupon el iva
							$nTotalSinTax += $sValue;
							$nTotalConTax += $sValue * $sTaxAux;

							// Restamos los ivas
							$order->info['tax_groups'][$sTax] -= ($nTotalConTax - $nTotalSinTax);
						}
						...
						// $this->output[] = array( 'title' => $order->coupon->format_display( $tax_group ) . ':', 'text' => $text, 'value' => $display_type . $discount );
					}
				}
			}
			...
			if (MODULE_ORDER_TOTAL_DISCOUNT_COUPON_DISPLAY_TAX == 'Display discounted tax in separate line' && is_array($order->coupon->discount_tax)) {
				...
				$this->output[] = ['title' => MODULE_ORDER_TOTAL_DISCOUNT_COUPON_TAX_NOT_APPLIED . ':',
								   'text'  => $text,
	...
	public function install() {
		...
		tep_db_query("CREATE TABLE IF NOT EXISTS discount_coupons_to_groups (
                      coupons_id VARCHAR(32) NOT NULL DEFAULT '',
	...
	public function keys() {
		return ['MODULE_ORDER_TOTAL_DISCOUNT_COUPON_STATUS',
				'MODULE_ORDER_TOTAL_DISCOUNT_COUPON_SORT_ORDER',
				'MODULE_ORDER_TOTAL_DISCOUNT_COUPON_DISPLAY_TYPE',
				'MODULE_ORDER_TOTAL_DISCOUNT_COUPON_DISPLAY_SUBTOTAL',
				'MODULE_ORDER_TOTAL_DISCOUNT_COUPON_DISPLAY_TAX',

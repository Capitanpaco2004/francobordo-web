...
class bizum {
	...
	function before_process() {
		...
		if ($this->fbEsNotificacion()) {
			...
			if ($sFbRef === '') {
				@error_log('[FB-REDSYS-REPLAY] notificacion sin Ds_Order legible module=bizum customer=' . (int)$customer_id);
			}
		}
		...
		if (!empty( $_POST ) ) {//URL DE RESP. ONLINE
			...
			if ($this->fbEsNotificacion() && $sFbRef !== '') {
				...
				if (empty($aFbLock['l'])) {
					...
					@error_log('[FB-REDSYS-REPLAY] LOCK no obtenido module=bizum ref=' . $sFbRef . ' customer=' . (int)$customer_id);
					$this->fbNotifRespuesta(503, 'REINTENTAR');
					die('REINTENTAR');
				}

				$rFbDup = tep_db_query("select id, orders_id from redsys_payment_movements where module = 'bizum' and reference = '" . tep_db_input($sFbRef) . "' and admin_id = 0 and value > 0 order by id desc limit 1");
				if ($aFbDup = tep_db_fetch_array($rFbDup)) {
					...
					@error_log('[FB-REDSYS-REPLAY] referencia YA procesada module=bizum ref=' . $sFbRef . ' movimiento=' . (int)$aFbDup['id'] . ' pedido=' . (int)$aFbDup['orders_id'] . ' customer=' . (int)$customer_id);
	...
	function after_process() {
			...
			if ($nFbCobrado !== null && is_object($order) && isset($order->info['total'])) {
				...
				if (abs($nFbCobrado - $nFbPedido) > 0.005) {
					@error_log('[FB-REDSYS-DESCUADRE] pedido=' . (int)$insert_id
						. ' module=bizum ref=' . $this->fbNotifReferencia()
		...
		if (tep_session_is_registered('cartID')) {
            ...
            if (!empty($_POST) && isset($_POST["Ds_MerchantParameters"])) {
                ...
                $sFbRef = $this->fbNotifReferencia();
                if ($sFbRef !== '') {
                    tep_db_query("update redsys_payment_movements set orders_id = " . (int)$insert_id . " where module = 'bizum' and reference = '" . tep_db_input($sFbRef) . "' and admin_id = 0 and value > 0 and orders_id = 0 order by id desc limit 1");
                } else {
                    $sql = sprintf(
                        'UPDATE redsys_payment_movements SET orders_id = %d WHERE customer_id = %d AND module = "bizum" ORDER BY id DESC LIMIT 1',
                        $insert_id,
                        $customer_id
                    );
                    tep_db_query($sql);
	...
	function keys() {
		return array( 'MODULE_PAYMENT_BIZUM_STATUS',

...
class paypal_applepay {
    ...
    public function before_process() {
        ...
        if ( $sStatus !== 'COMPLETED' ) {
            @error_log('[paypal_applepay] estado no COMPLETED order=' . $sOrderId . ' status=' . $sStatus . ' customer=' . (int)$customer_id);
        ...
        if ( strpos($sReference, 'cust-' . (int)$customer_id . '-') !== 0 ) {
            @error_log('[paypal_applepay] reference_id ajeno order=' . $sOrderId . ' ref=' . $sReference . ' customer=' . (int)$customer_id);
        ...
        // con el pedido en COMPLETED, caso real observado con Apple Pay).
        $aPayments = \PayPalRest\Client::listSettledPayments($aOrder, $sIntent);
        if ( empty($aPayments) ) {
            @error_log('[paypal_applepay] sin cobro registrado order=' . $sOrderId . ' intent=' . $sIntent . ' customer=' . (int)$customer_id);
        ...
        if ( $aPayment === null ) {
            @error_log('[paypal_applepay] capture_id no confirmado por PayPal order=' . $sOrderId . ' post=' . $sCaptureId . ' customer=' . (int)$customer_id);
        ...
        if ( ! \PayPalRest\Client::verifyCapturedAmount($aOrder, tep_round($order->info['total'], 2), 'EUR', $sIntent) ) {
            @error_log('[paypal_applepay] importe NO coincide order=' . $sOrderId . ' total_esperado=' . tep_round($order->info['total'], 2));
        ...
        if ( tep_db_num_rows($rReplay) > 0 ) {
            ...
            @error_log('[paypal_applepay] REPLAY order=' . $sOrderId . ' ya usado en pedido ' . (int)$aReplay['orders_id'] . ' customer=' . (int)$customer_id);
        ...
        if ( tep_db_num_rows($rReplay2) > 0 ) {
            ...
            @error_log('[paypal_applepay] REPLAY capture=' . $aPayment['id'] . ' ya usado en pedido ' . (int)$aReplay2['orders_id'] . ' customer=' . (int)$customer_id);
            tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . urlencode('Este pago de PayPal ya se ha utilizado en un pedido anterior.'), 'SSL'));
        }

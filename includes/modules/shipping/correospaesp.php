<?php
/*
  $Id: correospaesp.php, v1.00 2003/11/29 12:00:00 jmtorne Exp $
...
// class constructor
    function __construct() {
      global $order;
      $this->code = 'correospaesp';
      $this->icon = DIR_WS_ICONS . 'correos.png';
      $this->title = MODULE_SHIPPING_CORREOSPAESP_TEXT_TITLE;
      $this->description = MODULE_SHIPPING_CORREOSPAESP_TEXT_DESCRIPTION;
      $this->sort_order = MODULE_SHIPPING_CORREOSPAESP_SORT_ORDER;
      $this->tax_class = MODULE_SHIPPING_CORREOSPAESP_TAX_CLASS;
      $this->enabled = ((MODULE_SHIPPING_CORREOSPAESP_STATUS == 'True') ? true : false);
      $this->types = array('Normal' => MODULE_SHIPPING_CORREOSPAESP_TEXT_WAY1);
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_CORREOSPAESP_ZONE > 0) && is_object($order) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_CORREOSPAESP_ZONE . "' and zone_country_id = '" . $order->delivery['country']['id'] . "' order by zone_id");
...
// class methods
	...
	//Normal
	$table_cost = preg_split("/[:,]/" , MODULE_SHIPPING_CORREOSPAESP_COST);
	...
	$shipping['Normal'] = $shipping1 * $shipping_num_boxes;
        $this->quotes = array('id' => $this->code,
                            'module' => MODULE_SHIPPING_CORREOSPAESP_TEXT_TITLE );
	if($method)
	{
		$this->quotes['methods'][]=array('id' =>  $method,
						'title' => $this->types[$method],
						'cost' => $shipping[$method] + MODULE_SHIPPING_CORREOSPAESP_HANDLING);
	}else{
		foreach($this->types as $type=>$txtType)
		{
		$this->quotes['methods'][]=array('id' =>  $type,
						'title' => $txtType,
						'cost' => $shipping[$type] + MODULE_SHIPPING_CORREOSPAESP_HANDLING);
		}
	}
	if ($this->tax_class > 0) {
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_SHIPPING_CORREOSPAESP_STATUS'");
        $this->_check = tep_db_num_rows($check_query);
      }
      return $this->_check;
    }
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Activación de envió mediante Paquetería Urgente?', 'MODULE_SHIPPING_CORREOSPAESP_STATUS', 'True', '¿Quiere ofrecer esta forma de envío?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Tarifas Paquetería Urgente', 'MODULE_SHIPPING_CORREOSPAESP_COST', '2:4.3,5:5.00,10:5.90,15:8.20,20:10.00', 'Tarifas Paquetería Urgente vigentes durante el año 2005.', '6', '0', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Gastos de manipulación', 'MODULE_SHIPPING_CORREOSPAESP_HANDLING', '0', 'Coste manipulación.', '6', '0', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tipo de impuesto indirecto', 'MODULE_SHIPPING_CORREOSPAESP_TAX_CLASS', '0', 'Use el IVA como tipo de impuesto o bien no desglose impuestos en los envios.', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Zona de envío', 'MODULE_SHIPPING_CORREOSPAESP_ZONE', '0', '.', '6', '0', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Prioridad', 'MODULE_SHIPPING_CORREOSPAESP_SORT_ORDER', '0', 'Orden de aparición.', '6', '0', now())");
    }
    function remove() {
      tep_db_query("delete from " . TABLE_CONFIGURATION . " where configuration_key in ('" . implode("', '", $this->keys()) . "')");
    }
    function keys() {
      return array('MODULE_SHIPPING_CORREOSPAESP_STATUS', 'MODULE_SHIPPING_CORREOSPAESP_COST', 'MODULE_SHIPPING_CORREOSPAESP_HANDLING', 'MODULE_SHIPPING_CORREOSPAESP_TAX_CLASS', 'MODULE_SHIPPING_CORREOSPAESP_ZONE', 'MODULE_SHIPPING_CORREOSPAESP_SORT_ORDER');
    }
  }
?>

<?php
	...
	require( DIR_WS_LANGUAGES . $language . '/mejorar_precio.php' );
	...
	if( tep_db_num_rows( $aDatos ) > 0 )
	{
		...
		if( $_SERVER['REQUEST_METHOD'] == 'POST' )
		{
			...
			{
				$bError = true;
				$messageStack->add( 'error', MEJORAR_PRECIO_ERROR_EMAIL_TELEFONO, 'error', 'true' );
			}
			...
			{
				$bError = true;
				$messageStack->add( 'error', MEJORAR_PRECIO_ERROR_EMAIL, 'error', 'true' );
			}
			...
			{
				$bError = true;
				$messageStack->add( 'error', MEJORAR_PRECIO_ERROR_MENSAJE, 'error', 'true' );
			}
			...
			{
				// Asunto email
				$sAsuntoEmail = sprintf( MEJORAR_PRECIO_SEND_EMAIL_SUBJECT, $aProducto['products_name'], $sPostNombre );
				...
				$email = ' <table style="line-height: 20px; font-size: 15px; font-family: Arial; letter-spacing: 0.2px; color: #5f5f5f;" border="0" cellspacing="0" cellpadding="0">
								<tr>
									<td width="162" style="vertical-align: top; border-top: 1px solid #e4e4e4; border-right: 1px solid #e4e4e4; padding: 13px 0 13px 16px; text-align: left;">
										<font style="font-weight: bold">' . MEJORAR_PRECIO_INPUT_NOMBRE . '</font>
									</td>
									<td style="padding: 13px 23px; border-top: 1px solid #e4e4e4">' . $sPostNombre . '</td>
								</tr>
								<tr>
									<td width="162" style="vertical-align: top; border-top: 1px solid #e4e4e4; border-right: 1px solid #e4e4e4; padding: 13px 0 13px 16px; text-align: left;">
										<font style="font-weight: bold">' . MEJORAR_PRECIO_INPUT_EMAIL . '</font>
									</td>
								...
								<tr>
									<td width="162" style="vertical-align: top; border-top: 1px solid #e4e4e4; border-right: 1px solid #e4e4e4; padding: 13px 0 13px 16px; text-align: left;">
										<font style="font-weight: bold">' . MEJORAR_PRECIO_INPUT_TELEFONO . '</font>
									</td>
									<td style="padding: 13px 23px; border-top: 1px solid #e4e4e4">' . $sPostTelefono . '</td>
								</tr>
								<tr>
									<td width="162" style="vertical-align: top; border-top: 1px solid #e4e4e4; border-right: 1px solid #e4e4e4; padding: 13px 0 13px 16px; text-align: left;">
										<font style="font-weight: bold">' . MEJORAR_PRECIO_ASUNTO . '</font>
									</td>
									<td style="padding: 13px 23px; border-top: 1px solid #e4e4e4;">' . $sAsuntoEmail . '</td>
								</tr>
								<tr>
									<td width="162" style="vertical-align: top; border-top: 1px solid #e4e4e4; border-right: 1px solid #e4e4e4; padding: 13px 0 13px 16px; text-align: left;">
										<font style="font-weight: bold">' . MEJORAR_PRECIO_ENLACE . '</font>
									</td>
								...
								<tr>
									<td width="162" style="vertical-align: top; border-top: 1px solid #e4e4e4; border-bottom: 1px solid #e4e4e4; border-right: 1px solid #e4e4e4; padding: 13px 0 13px 16px; text-align: left;">
										<font style="font-weight: bold">' . MEJORAR_PRECIO_CONSULTA . '</font>
									</td>
				...
				$messageStack->addSession( 'success', MEJORAR_PRECIO_EXITO, 'success' );
				tep_redirect( tep_href_link( 'mejorar_precio.php' ) );
			}
		}
		...
		echo '<div id="ltbx-cnsl">';
			echo '<div class="titl">' . sprintf( MEJORAR_PRECIO_TITULO, $aProducto['products_name'] ) . '</div>';
			...
			echo '<div id="rcmd-emil-cntd" style="position: relative;">';
				echo '<form id="contact_us_form" method="post" action="' . tep_href_link( 'mejorar_precio.php?id=' . $sGetId ) . '" onsubmit="return app.get(\'alert\').formAjax(this);">';
					echo '<div class="text">' . MEJORAR_PRECIO_TEXTO . '</div>';

					echo '<p>
						<label for="nombre">' . MEJORAR_PRECIO_INPUT_NOMBRE . '</label>
						<input value="' . $sPostNombre . '" type="text" id="nombre" tabindex="1" autocomplete="off" name="nombre" />
					</p>';

					echo '<p>
						<label for="email">' . MEJORAR_PRECIO_INPUT_EMAIL . '</label>
						<input value="' . $sPostEmail . '" type="text" id="email" tabindex="2" autocomplete="off" name="email" />
					</p>';

					echo '<p>
						<label for="telefono">' . MEJORAR_PRECIO_INPUT_TELEFONO . '</label>
						<input value="' . $sPostTelefono . '" type="text" id="telefono" tabindex="3" autocomplete="off" name="telefono" />
					</p>';

					echo '<p>
						<label for="mensaje">' . MEJORAR_PRECIO_INPUT_MENSAJE . '</label>
					...
					echo '<input id="submit-form" type="submit" class="sbmt" tabindex="5" value="' . MEJORAR_PRECIO_INPUT_BOTON . '">';

...
$sSql = 'SELECT ' . SQL_SELECT . ' p.products_status, p.products_min_order_qty, s.expires_date, GROUP_CONCAT( CONCAT( p2c.categories_id, ", ") ORDER BY p2c.categories_id ASC SEPARATOR " ") grcats, m.manufacturers_image, m.manufacturers_name, p.products_weight, p.products_pdfupload, p.products_fileupload, p.products_id, pd.products_tab_1, pd.products_tab_2, pd.products_tab_3, pd.products_tab_4, pd.products_tab_5, pd.products_tab_6, pd.products_name, pd.products_description, p.products_model, p.products_youtube, p.products_quantity, p.check_stock, p.products_image, p.products_subimages, pd.products_url, p.products_price, p.products_tax_class_id, p.products_date_added, p.products_date_available, p.manufacturers_id, p.products_ship_free, p.products_bundle, p.products_youtube, p. sold_in_bundle_only
			 FROM products p
...
if ($product_check['total'] < 1 || $product_check['products_status'] == '0') {
	...
	// Banner informativo PayPal Pay Later (solo IP Francobordo) -- define $sPayPalPayLaterBanner
	include(DIR_WS_INCLUDES . 'paypal_paylater_banner.php');

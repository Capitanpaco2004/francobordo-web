...
*/
  ...
  $showManufacturers = '';
  if (tep_not_null($settings['heading_manufacturers']) && count($manufacturersArray) > 0) {
      ...
      if ($cnt > 0){
        ...
        for ($p = 0; $p < $cnt; ++$p) {
          $pA = $manufacturersArray[$i]['productArray'][$p]; //makes it more readable
     		 $showManufacturers .= '<nobr><li><a class="sitemapProducts" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $pA['link']) .'" title="' . $pA['anchor_text'] . '">' . $pA['text'] . '</a></li></nobr>';
        }
        $showManufacturers .= '</ul>';
      }
    }
  ...
  $showInfoBoxes = '';
  if (tep_not_null($settings['heading_standard_boxes']) && count($boxDataArray) > 0) {
    ...
    for ($i = 0; $i < $pageCount; ++$i)
 			$showInfoBoxes .= '<nobr><li><a class="sitemap" href="' . tep_href_link($boxDataArray[$i]['link']) .'" title="' . $boxDataArray[$i]['anchor_text'] . '">' . $boxDataArray[$i]['text'] . '</a></li></nobr>';
    $showInfoBoxes .= '</ul></ul></td></tr>';
  }

  /****************** DISPLAY STANDARD PAGES *********************/
  $showPages = '';
  if (tep_not_null($settings['heading_standard_pages']) && count($pagesArray) > 0) {
  ...
  $leftWidth  = (tep_not_null($rightColDisplay) ? 'width="50%"' : 'width="100%"');
  $rightWidth = (tep_not_null($leftColDisplay) ? 'width="50%"' : 'width="100%"');

...
!function ($){
	dropdownClass = function(dmElement, aArguments)
	{
		// Si el elemento tiene ya dropdown pasamos de el
		if( dmElement.data("dropdown") != undefined )
			return false;
		...
		this.nTime = $(dmElement).attr('data-time') || 220;

		// Evento a realizar
		this.sEvent = $(dmElement).attr('data-event') || "click";
		...
		this.open = function(e)
		{
			...
			self.tmTimer = setTimeout(function()
			{
				...
				if( dmFlot.css("position") == "fixed" )
				{
					...
					dmFlot.css({ top: aAux.top - $(window).scrollTop() + $(dmElement).height(), left: aAux.left, width: $(this).width() });
					
					// Cuando realizamos scroll se posicione
					$(window).bind( "scroll.dropdown", function()
					{
						dmFlot.css({ top: aAux.top - $(window).scrollTop() + $(dmElement).height(), left: aAux.left });	
					});
		...
		this.close = function()
		{
			// Si el evento es hover eliminamos el timer para desplegar
			if( self.sEvent == "hover" )
